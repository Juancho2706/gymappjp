//
//  EvaTimerAttributes.swift  (copia B — pod del módulo nativo)
//
//  ⚠️ COPIA SINCRONIZADA — EDITAR AMBAS ⚠️
//  Gemela byte-idéntica de:
//      apps/mobile/targets/eva-timer-activity/_shared/EvaTimerAttributes.swift
...
//    1. Este POD (`EvaLiveActivity`, autolinkeado desde `modules/`) es quien crea la actividad cuando
//       el JS llama `start()`. Un pod se compila ANTES que el target de la app, así que NO puede
//       importar símbolos de la app.
...
//  por eso este pod lleva su propia copia. Duplicar el struct es la vía que Apple tolera —el matching
