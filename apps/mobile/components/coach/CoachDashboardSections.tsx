...
// Acá vivían el modal de bienvenida Free (`MobileFreeWelcomeModal`) y el checklist de 4 pasos
// (`MobileOnboardingChecklist` + sus 6 bloques). Los retiró la decisión del owner (2026-08-22):
// «no podemos tener varios onboardings en una sola área». La guía v2 —pantalla propia
// `/coach/guia` + píldora flotante— ES la bienvenida, en la app y en la web.
...
/**
 * La «Guía de inicio» como chip de este dashboard se RETIRÓ (owner 2026-08-22, paridad con la
 * web): la guía vive en su pantalla propia `app/coach/guia.tsx` y en el panel queda la píldora
 * flotante `components/coach/GuidePill.tsx`, montada en `app/coach/(tabs)/_layout.tsx`.
 *
 * Con él se va su estado local: el chip persistía «saltar guía» y el tilde manual de marca SOLO en
 * AsyncStorage (`eva_coach_guide_chip_hidden:{coachId}`), así que reinstalar la app resucitaba la
 * guía y ocultarla en la web no la ocultaba en el teléfono. La pantalla nueva persiste contra
