...
// Default = web (sonido ON). El cache se sobreescribe tras hidratar.
const cache: PrefsCache = { muted: false, sound: 'digital', volume: 1 }
let hydrated = false
...
export async function hydrateRestTimerPrefs(): Promise<void> {
  ...
  hydrated = true
  try {
    const [muted, sound, volume] = await Promise.all([
