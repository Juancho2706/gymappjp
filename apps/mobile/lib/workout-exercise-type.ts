 ...
 * Port verbatim de apps/web/src/lib/workout-exercise-type.ts (función pura, sin Next/Supabase).
 *
 ...
 *
 * NOTA anti-drift: idealmente vive en un package compartido (@eva/*). Mientras tanto es un
 * port verbatim — si cambian las fórmulas en web, actualizar acá.
 */

import type { DistanceUnit, ExerciseType } from './plan-builder/types'
import type { IntervalConfig } from './cardio'
...
export function legacyRepsSummaryFor(block: TypedBlockFields, type: ExerciseType): string {
  ...
  if (type === 'cardio') {
    ...
    if (interval) {
      ...
      if (work) return truncate20(`${interval.repeats ?? 1}×${work}${zone}`)
    }
    const zone = block.hr_zone != null ? ` Z${block.hr_zone}` : ''
