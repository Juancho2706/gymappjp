...
export const PORTIONS_COPY = {
   ...
   * Vive en el mismo sheet del picker. Espejo EXACTO de la tabla web (FD6a).
   */
  groupEditor: {
  ...
  /**
   * Bloque opcional "Equivalencia de porciones" del alta de alimentos del coach (clasificar
  ...
  foodEquivalence: {
    ...
    preview: (grupo: string, gramos: string, medida: string | null) =>
      medida ? `1 porción de ${grupo} = ${gramos} g (${medida})` : `1 porción de ${grupo} = ${gramos} g`,
