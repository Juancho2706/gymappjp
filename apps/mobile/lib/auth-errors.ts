...
export function translateAuthError(raw?: string | null): string {
  ...
  if (m.includes('too many') || m.includes('rate limit')) return 'Demasiados intentos. Esperá unos minutos.'
  if (m.includes('network') || m.includes('failed to fetch') || m.includes('fetch')) return 'Sin conexión. Revisá tu internet.'
