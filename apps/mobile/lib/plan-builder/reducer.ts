// Ported 1:1 from web: apps/web/src/app/coach/builder/[clientId]/hooks/usePlanBuilder.ts
// Pure React (no DOM/dnd-kit) → runs identically on RN. Only change: local arrayMove.
...
function builderReducer(state: DayState[], action: BuilderAction): DayState[] {
  switch (action.type) {
    ...
    case 'ADD_BLOCK':
      return state.map((d) =>
        d.id === action.payload.dayId ? { ...d, blocks: [...d.blocks, action.payload.block] } : d
      )

    case 'REMOVE_BLOCK':
      return state.map((d) =>
        d.id === action.payload.dayId ? { ...d, blocks: d.blocks.filter((b) => b.uid !== action.payload.uid) } : d
      )
    ...
    case 'TRANSFER_BLOCK': {
      ...
      return state.map((d) => {
        if (d.id === activeDayId) return { ...d, blocks: d.blocks.filter((_, i) => i !== activeBlockIndex) }
        if (d.id === overDayId) return { ...d, blocks: [...d.blocks, { ...activeBlock, dayId: overDayId }] }
        return d
      })
    }
    ...
    case 'TOGGLE_SUPERSET': {
      ...
      return state.map((d) => {
        ...
        if (block.superset_group) {
          ...
          return {
            ...d,
            blocks: d.blocks.map((b, i) => (b.uid === uid || i === idx + 1 ? { ...b, superset_group: groupToUse } : b)),
          }
        }
      })
    }
