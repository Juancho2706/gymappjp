...
export const getCoachTeamOverview = cache(async (activeTeamId: string): Promise<{ userId: string | null; teams: TeamOverview[] }> => {
    ...
    const { data: teams } = await supabase
        .from('teams')
        .select('id, name, slug, seat_limit, owner_coach_id, invite_code, primary_color, brand_secondary_color, logo_url, logo_url_dark, accent_light, accent_dark, neutral_tint, splash_bg_color, loader_text, loader_text_color, loader_icon_mode, use_custom_loader, enabled_modules')
        .eq('id', activeTeamId)
        .is('deleted_at', null)
