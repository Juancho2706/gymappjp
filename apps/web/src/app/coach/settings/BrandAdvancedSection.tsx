...
/** Sección "Branding avanzado (Pro)" del white-label v2: color2 + fuente + dark + loader (variante +
...
export function BrandAdvancedSection({ tier, primaryColor, value, onChange, loader, onLoaderChange, logoUrl }: Props) {
    ...
    // Tema derivado en vivo (mismo motor que el render real) → guardia de contraste.
    const theme = useMemo(() => {
    ...
    const loaderPalette = generateBrandPalette(HEX_RE.test(primaryColor) ? primaryColor : '#10B981')
        ...
        <div className={CARD} data-tour-id="brand-advanced">
            ...
            >
                ...
                <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted">Color secundario, fuente, acento por modo y pantalla de carga.</p>
                </div>
            ...
            {open && (
                <div className="mt-5 space-y-6">
                    ...
                    {/* ── Pantalla de carga (variante + texto/ícono/color) — UNA sola sección de loader ── */}
                    <div className="space-y-3 pt-1">
                        <div className="flex items-center gap-2">
                            <Loader2 className="w-3.5 h-3.5 text-muted" />
                            <span className="text-sm font-semibold text-strong">Pantalla de carga</span>
                        </div>
                        <p className="text-xs text-muted">Animación que ven tus alumnos al cargar la app o navegar entre páginas.</p>
                        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                        ...
                        <label className="flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-border p-3">
                            <span className="min-w-0">
                                <span className="block text-sm font-semibold text-strong">Usar texto personalizado</span>
                                <span className="block text-xs text-muted">Muestra tu marca en vez de &quot;EVA&quot; en la animación.</span>
                            </span>
                            <button
                                type="button"
                                role="switch"
                                aria-checked={loader.useCustomLoader}
                                onClick={() => onLoaderChange({ useCustomLoader: !loader.useCustomLoader })}
                                className={`relative h-6 w-11 shrink-0 rounded-full transition-colors ${loader.useCustomLoader ? 'bg-primary' : 'bg-border'}`}
                            >
                                <span className={`absolute top-0.5 h-5 w-5 rounded-full bg-white shadow transition-all ${loader.useCustomLoader ? 'left-[22px]' : 'left-0.5'}`} />
                            </button>
                        </label>
                            ...
                            <div className="space-y-4">
                                <div className="space-y-1.5">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => onLoaderChange({ loaderText: e.target.value.toUpperCase() })}
                                        maxLength={10}
                                        placeholder="EVA"
                                    ...
                                    />
                                    <p className="text-xs text-muted">Máximo 10 caracteres. Se transforma automáticamente a mayúsculas.</p>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => onLoaderChange({ loaderTextColor: loader.loaderTextColor || primaryColor })}
                                            className={`flex flex-col items-center gap-1 rounded-xl border-2 p-3 transition-all ${
                                                loader.loaderTextColor !== '' ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/40'
                                            }`}
                                        >
                                            <span className="text-xl font-extrabold" style={{ color: loader.loaderTextColor || primaryColor }}>
                                                {(loader.loaderText || 'EVA').toUpperCase()}
                                            </span>
                                    ...
                                    {loader.loaderTextColor !== '' && (
                                        <div className="flex items-center gap-3 pt-2">
                                            <input
                                                type="color"
                                                value={HEX_RE.test(loader.loaderTextColor) ? loader.loaderTextColor : primaryColor}
                                    ...
                                    <p className="text-[10px] text-muted">Gradiente: el mismo estilo animado que usa EVA. Color sólido: tu color de marca con animación de pulso.</p>
                                </div>
                            </div>
                        )}
                    </div>

                    {/* ── Guardia WCAG (sin preview propio: la vista previa canónica es la del teléfono) ── */}
                    {failing.length === 0 ? (
