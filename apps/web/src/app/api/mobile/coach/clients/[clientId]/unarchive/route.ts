...
import { unarchiveClient } from '@/services/client/client-archive.service'
import {
  archiveActor,
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ clientId: string }> }) {
  ...
  const result = await unarchiveClient(createServiceRoleClient(), archiveActor(context), clientId)
  if (!result.ok) return archiveErrorResponse(result)
  void sendArchiveLifecycleEmail({ context, client: result.client, kind: 'unarchived' }).catch(() => {})
