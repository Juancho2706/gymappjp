        ...
        <div className="flex items-center justify-between gap-3 rounded-lg bg-surface-sunken px-3 py-2">
            ...
            <div className="flex shrink-0 gap-1">
                <button onClick={() => { navigator.clipboard.writeText(value); setCopied(true); setTimeout(() => setCopied(false), 2000) }}
                    className="rounded p-1 text-muted hover:text-strong transition-colors" title="Copiar">
                    {copied ? <CheckCircle2 className="h-3.5 w-3.5 text-[var(--success-500)]" /> : <Copy className="h-3.5 w-3.5" />}
                </button>
                {href && (
                    <a href={href} target="_blank" rel="noopener noreferrer" className="rounded p-1 text-muted hover:text-strong transition-colors" title="Abrir">
    ...
    return (
        <Sheet open={open} onOpenChange={(o) => !o && handleClose()}>
            <SheetContent className="w-full sm:max-w-lg bg-surface-card border-subtle text-strong overflow-y-auto">
                <SheetHeader>
                    <SheetTitle className="text-strong">{succeeded ? 'Equipo creado' : 'Nuevo Equipo'}</SheetTitle>
                </SheetHeader>
                ...
                {succeeded ? (
                    <div className="mt-6 space-y-4">
                        <div className="flex items-center gap-2 rounded-lg border border-[var(--success-500)]/30 bg-[var(--success-500)]/15 px-3 py-2.5">
                            ...
                            <p className="text-sm text-[var(--success-500)]">Equipo creado. El owner ya puede sumar al resto desde su panel «Equipo».</p>
                        </div>
                    ...
                    <form action={(fd) => startTransition(() => formAction(fd))} className="mt-6 space-y-4">
                        ...
                        <div>
                            <Label className="text-body text-xs">Slug (opcional)</Label>
                            <Input name="slug" placeholder="auto desde el nombre" className={`${fieldCls} font-mono`} maxLength={46} />
                            <p className="mt-1 text-[10px] text-muted">Los alumnos entran por /t/[slug]/login</p>
                        </div>
                        <div>
                        ...
                        <div className="rounded-lg border border-subtle p-3">
                            <p className="text-xs font-medium text-body">Owner del equipo</p>
                            ...
                            {ownerMode === 'new' && (
                                <div className="mt-3">
                                    <Label className="text-body text-xs">Nombre del owner</Label>
                                    <Input name="owner_full_name" placeholder="Ani Madrid" className={fieldCls} maxLength={120} />
                                </div>
                            )}
                            <div className="mt-3">
                                <Label className="text-body text-xs">Email del owner</Label>
                            ...
                            {ownerMode === 'new' && (
                                <div className="mt-3">
                                    ...
                                    <div className="mt-1 flex gap-2">
                                        <Input name="owner_temp_password" type="text" value={password} onChange={e => setPassword(e.target.value)} placeholder="Mín. 8 caracteres" className="bg-surface-sunken border-subtle text-strong font-mono placeholder:text-muted" minLength={8} />
                                        <button type="button" onClick={() => setPassword(generateTempPassword())} className="shrink-0 rounded-lg border border-subtle bg-surface-sunken px-2.5 text-muted hover:text-strong hover:border-strong transition-colors" title="Generar contraseña">
                            ...
                            <div className="mt-2 grid grid-cols-1 gap-2">
                                {MODULE_KEYS.map(key => (
                                    <label key={key} className="flex items-center gap-2 text-sm text-body">
                                        <input type="checkbox" name={`module_${key}`} className="h-4 w-4 rounded border-subtle bg-surface-sunken" />
                                        {MODULE_LABELS[key]}
                                    </label>
                                ))}
                            </div>
