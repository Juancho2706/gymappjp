...
export function AnnouncementEmailButton() {
    ...
    return (
        <div className="flex items-center gap-2">
            {isSuccess && (
                <span className="flex items-center gap-1.5 text-xs text-[var(--success-500)]">
                    <CheckCircle2 className="h-3.5 w-3.5" />
                    {result.sent} enviados{result.failed > 0 ? `, ${result.failed} fallidos` : ''}
                </span>
            )}
            {isError && (
                <span className="flex items-center gap-1.5 text-xs text-[var(--danger-500)]">
            ...
            <button
                ...
                className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-semibold transition-all ${
                    confirmed
                        ? 'border-[var(--warning-500)]/30 bg-[var(--warning-500)]/15 text-[var(--warning-500)] hover:bg-[var(--warning-500)]/15'
                        : 'border-subtle bg-surface-sunken text-body hover:text-[var(--sport-500)] hover:border-[var(--sport-500)]'
                } disabled:opacity-50`}
            >
                {isPending
