...
function ResendConfirmation({ uid }: { uid: string }) {
    ...
    // `message` (no `error`): la cuenta ya quedó activa al pedir el reenvío —el Server Action la
    // sanó porque GoTrue ya tenía el email confirmado—, así que esto es una buena noticia, no un
    // fallo, y se pinta en el mismo tono que el reenvío exitoso.
    if (state.message) {
        return (
            <p className="mt-[18px] text-[12.5px] text-[var(--success-700)]">{state.message}</p>
        )
    }
