 ...
 * nadie perdió.
 *
 * Vive en un módulo propio (y no dentro de `LogSetForm.tsx`) por dos razones: las DOS filas
 * —`StrengthLogSetForm` y `TypedLogSetRow`— reconcilian su error por separado y tienen que reportar
 * exactamente igual, y así el guard es testeable sin montar el ejecutor entero.
 *
 * Espejo exacto del helper RN (`apps/mobile/components/alumno/workout/v3/hold-autolog-report.ts`):
 * mismo tag `area: 'hold-autolog'` y mismas tres claves de `extra`, o los dos clientes no se pueden
 * contar juntos. Lo único que cambia es el SDK (`@sentry/nextjs` vs `@sentry/react-native`).
 *
 ...
 * capturó (útil para los tests del guard). NUNCA lanza: un Sentry sin inicializar no puede tumbar la
 * reconciliación de una serie.
 */
