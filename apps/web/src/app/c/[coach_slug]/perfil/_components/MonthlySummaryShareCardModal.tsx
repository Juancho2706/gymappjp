...
import {
    readShareCardBrand,
    renderMonthlySummaryCardToBlob,
    type MonthlySummaryCardData,
} from '@/lib/workout-pr-card-canvas'

interface MonthlySummaryShareCardModalProps {
    data: MonthlySummaryCardData
    onClose: () => void
}

/**
 * Preview + acciones (Compartir / Guardar) de la share-card de RESUMEN MENSUAL del alumno. La imagen
 * se genera 100% en el cliente (<canvas>, zero-server). Hermano de ProgressShareCardModal (misma UX).
 */
export function MonthlySummaryShareCardModal({ data, onClose }: MonthlySummaryShareCardModalProps) {
    const reducedMotion = useReducedMotion()
    const [previewUrl, setPreviewUrl] = useState<string | null>(null)
    const [busy, setBusy] = useState(false)
    const blobRef = useRef<Blob | null>(null)
    const fileName = 'mi-resumen-mensual.png'
    ...
    useEffect(() => {
        ...
        ;(async () => {
            ...
            const blob = await renderMonthlySummaryCardToBlob(data, brand)
            if (cancelled) return
            if (!blob) {
    ...
    async function handleShare() {
        ...
        try {
            ...
            const text = 'Mi resumen del mes 📅'
            if (canShareFiles([file])) {
                await share({ files: [file], title: 'Mi resumen del mes', text })
            } else {
                handleDownload()
            }
        } finally {
    ...
    return createPortal(
        <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Compartir tu resumen mensual"
            onClick={onClose}
        >
            <motion.div
            ...
            >
                <div className="relative aspect-[1080/1350] w-full overflow-hidden rounded-card border border-white/10 bg-[var(--ink-950,#0B0E13)] shadow-2xl">
                    {previewUrl ? (
                        ...
                        <img src={previewUrl} alt="Vista previa de tu resumen mensual" className="h-full w-full object-cover" />
                    ) : (
