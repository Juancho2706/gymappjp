...
import {
    readShareCardBrand,
    renderProgressCardToBlob,
    type ProgressCardData,
} from '@/lib/workout-pr-card-canvas'

interface ProgressShareCardModalProps {
    data: ProgressCardData
    onClose: () => void
}

/**
 * Preview + acciones (Compartir / Guardar) de la share-card de PROGRESO del alumno. La imagen se genera
 * 100% en el cliente (<canvas>, zero-server) a partir de las métricas que el perfil ya tiene. Se portalea
 * a document.body. Hermano de PRShareCardModal (misma UX), separado para no acoplar el módulo de workout.
 */
export function ProgressShareCardModal({ data, onClose }: ProgressShareCardModalProps) {
    const reducedMotion = useReducedMotion()
    const [previewUrl, setPreviewUrl] = useState<string | null>(null)
    const [busy, setBusy] = useState(false)
    const blobRef = useRef<Blob | null>(null)
    const fileName = 'mi-progreso.png'
    ...
    async function handleShare() {
        ...
        try {
            ...
            if (canShareFiles([file])) {
                await share({ files: [file], title: 'Mi progreso', text })
            } else {
                handleDownload()
            }
        } finally {
    ...
    return createPortal(
        <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Compartir tu progreso"
            onClick={onClose}
        >
            <motion.div
            ...
            >
                <div className="relative aspect-[1080/1350] w-full overflow-hidden rounded-card border border-white/10 bg-[var(--ink-950,#0B0E13)] shadow-2xl">
                    {previewUrl ? (
                        ...
                        <img src={previewUrl} alt="Vista previa de tu progreso" className="h-full w-full object-cover" />
                    ) : (
