...
import {
    readShareCardBrand,
    renderStreakCardToBlob,
    type StreakCardData,
} from '@/lib/workout-pr-card-canvas'

interface StreakShareCardModalProps {
    data: StreakCardData
    onClose: () => void
}

/**
 * Preview + acciones (Compartir / Guardar) de la share-card de RACHA del alumno. La imagen se genera
 * 100% en el cliente (<canvas>, zero-server). Hermano de ProgressShareCardModal (misma UX).
 */
export function StreakShareCardModal({ data, onClose }: StreakShareCardModalProps) {
    const reducedMotion = useReducedMotion()
    const [previewUrl, setPreviewUrl] = useState<string | null>(null)
    const [busy, setBusy] = useState(false)
    const blobRef = useRef<Blob | null>(null)
    const fileName = 'mi-racha.png'
    ...
    async function handleShare() {
        ...
        try {
            ...
            if (canShareFiles([file])) {
                await share({ files: [file], title: 'Mi racha', text })
            } else {
                handleDownload()
            }
        } finally {
            ...
            >
                <div className="relative aspect-[1080/1350] w-full overflow-hidden rounded-card border border-white/10 bg-[var(--ink-950,#0B0E13)] shadow-2xl">
                    {previewUrl ? (
                        ...
                        <img src={previewUrl} alt="Vista previa de tu racha" className="h-full w-full object-cover" />
                    ) : (
