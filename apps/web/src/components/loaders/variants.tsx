...
function Icon({ iconSrc, iconSrcDark, px = 48 }: { iconSrc?: string; iconSrcDark?: string; px?: number }) {
    if (!iconSrc) return null
    return (
        <ThemedLogo light={iconSrc} dark={iconSrcDark} alt="" width={px} height={px} priority={false} className={styles.icon} style={{ width: px, height: px }} />
    )
}
...
function CompositeSymbol({ symbol, iconSrc, iconSrcDark, initial, px }: { symbol: LoaderSymbol; iconSrc?: string; iconSrcDark?: string; initial: string; px: number }) {
    if (symbol === 'logo' && iconSrc) {
        return <ThemedLogo light={iconSrc} dark={iconSrcDark} alt="" width={px} height={px} className={styles.icon} style={{ width: px, height: px }} priority={false} />
    }
...
export function CompositeLoaderView({ config, brandName, iconSrc, iconSrcDark, subtitle, size = 'lg' }: CompositeLoaderProps) {
    const word = (config.text?.trim() || brandName?.trim() || 'EVA').toUpperCase()
    const px = size === 'lg' ? 52 : 40
    const initial = word.charAt(0)
    const symbol = <CompositeSymbol symbol={config.symbol} iconSrc={iconSrc} iconSrcDark={iconSrcDark} initial={initial} px={px} />
