...
export interface ScannerRegistrationContext {
  ...
  /**
   * Ruta del Today del alumno para VOLVER tras registrar ("ver registro"). Es SOLO navegacion:
   * la revalidacion la deriva el servidor del header del proxy (NUT-006), nunca del cliente.
   */
  backHref: string
}
 ...
 * libre del catalogo (buildCatalogIntakePayload: source 'offplan', snapshot per-100 con
 * `macrosBasis` declarada, el RPC escala) con un solo cambio honesto: captureMethod 'barcode',
