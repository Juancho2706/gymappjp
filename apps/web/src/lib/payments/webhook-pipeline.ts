 ...
 *
 * Extraido TAL CUAL del handler MP (`app/api/payments/webhook/route.ts`) SIN cambio de comportamiento:
 * es una separacion de responsabilidades, no una reescritura. Toma un `provider` ya elegido + el
 * `payload` normalizado + el `notificationId`, y ejecuta todo lo que hoy es provider-agnostico aguas
 * abajo de `provider.processWebhook`: idempotencia por `subscription_events`, ramas money-safety
 * (recurring / one-shot add-on / tier-upgrade / refund-chargeback / canonico), hooks de add-ons y
 * snapshots. Lo reusan la ruta MP (auth token + x-signature) y la futura ruta Flow (auth propia),
 * cada una con `getPaymentsProvider('mercadopago' | 'flow')`.
 */
export type RunWebhookPipelineArgs = {
    provider: PaymentsProvider
    /** Payload ya normalizado por la ruta (MP: `buildPayload`; Flow: re-fetch firmado). */
    payload: unknown
    /** id de la notificacion (clave de dedup top-level). `null` si el gateway no la expone. */
...
export async function runWebhookPipeline(
    request: Request,
    { provider, payload, notificationId }: RunWebhookPipelineArgs
    ...
    if (result.isRecurringAuthorizedPayment) {
        ...
        if (recurringStatus === 'active') {
            ...
            // 200 (MP no reintenta); con el UPDATE acá arriba, un fallo del snapshot —o cualquier hipo
            // transitorio de DB— NUNCA puede saltarse la renovación del período ya pagado. Espejo del
            // patrón de la rama canónica (update del coach primero, hooks en try/catch que traga).
    ...
    if (result.eventKind === 'payment' && result.tierUpgrade) {
        ...
        try {
            ...
            if (result.providerPaymentId) {
                // Ola 1: snapshot del upgrade con atribución de gateway (`provider`). upsert directo (no vía
                // insertBillingSnapshot) con el mismo onConflict compuesto. Tira ante error → lo atrapa el
                // catch del upgrade (500 → MP reintenta, idempotente por payment id).
                ...
                const { error: snapErr } = await admin
                    ...
                    .upsert(snapshotRow, { onConflict: 'provider,provider_payment_id', ignoreDuplicates: true })
