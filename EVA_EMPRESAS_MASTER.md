...
> **Versión:** 2026-05-07 · rev. FINAL (síntesis rev.6 + Kimi Edition + análisis competitivo actualizado)
> **Estado:** planificación completa. Todas las decisiones resueltas. Sin código hasta que equipo apruebe.
...
> **Tipo de cambio referencia:** 1 USD = 942 CLP (mayo 2025)
...
| 22 | Análisis de precios — posicionamiento global | Estrategia |
...
**Criterio de éxito del piloto:** mínimo 1 coach del gym aceptó invitación + registró al menos 1 sesión de alumno en los primeros 14 días + el admin org ingresó al panel `/org/dashboard` al menos 2 veces en los primeros 7 días. Sin ese criterio, "piloto exitoso" es subjetivo.
...
| Plan | Costo/coach | Descuento vs individual | USD equivalente |
|------|-------------|------------------------|-----------------|
| Starter Gym | $17.998 | –40% | ~$19 USD |
| Pro Gym | $15.999 | –47% | ~$17 USD |
| Elite Gym | $13.499 | –55% | ~$14 USD |
...
→ Si el coach tenía suscripción retail previa, el trigger `trg_coach_billing_reset` restaura
  `billing_source = 'self'` automáticamente. El coach debe configurar nuevo método de pago.
```
...
├────────────────────────────────────────────────────────┤
│  Historial de pagos                                    │
│  15 abr 2026   $159.990 CLP   ✓ Confirmado            │
│  15 mar 2026   $159.990 CLP   ✓ Confirmado            │
│  15 feb 2026   $159.990 CLP   ✓ Confirmado            │
├────────────────────────────────────────────────────────┤
...
Checklist en /org/dashboard (desaparece al completar 5 pasos):
...
└── invite/
    └── accept/
        ├── page.tsx                  # RSC pública — verifica token + sesión activa con email coincidente
...
-- Ejecutar en staging: \d workout_logs
...
Fundador en /admin (< 5 min):
  ...
  5. Server action llama revalidateTag(['org-billing', orgId]) y
     revalidateTag(['org-dashboard', orgId]) → panel gym se actualiza
...
ORG_RENEWAL_EMAIL_DOMAIN=eva-app.cl   # from address fijo reduce riesgo de spoofing
...
**Opción A — pg_cron:** disponible en Supabase Pro+. Verificar plan antes con `SELECT * FROM pg_extension WHERE extname = 'pg_cron'`.
...
*Edge Function schedules tienen granularidad mínima de 1 hora. Para transiciones exactas de billing, documentar que la transición puede tardar hasta 60 min.*
...
Si gym confirma upgrade:
  1. Calcular diferencia prorrateada: (precio_nuevo - precio_actual) / 30 × días_restantes
     Cobrar mes completo si faltan <15 días; prorrateo lineal si faltan ≥15 días.
...
- Si un gym es SpA o SA y pide contrato con RUT empresa → tener respuesta preparada: "Actualmente operamos como personas naturales; el contrato es igualmente válido en Chile. Te enviamos los términos en formato PDF para tu revisión."
...
**Mecanismo de aceptación:** reforzado en dos niveles:
- Desde MVP, agregar checkbox en `/org/billing` o en email de bienvenida: "He leído y acepto los Términos de uso B2B". Guardar timestamp + IP como evidencia.
...
- `tax_id` (RUT org): **No recolectar `tax_id` hasta que Supabase Vault esté disponible y verificado en producción.** Si se habilita el campo en UI antes del cifrado, bloquear en middleware. Verificar con `SELECT * FROM vault.secrets LIMIT 1`. Fallback: pgcrypto `PGP_SYM_ENCRYPT`.
...
| MyPTHub whitelist app $157/mes como alternativa low-cost | Media | Bajo | El diferenciador no es el precio sino la white-label POR COACH. $157 da UNA app de marca de gym, igual que PT Distinction. |
...
| 17 | tax_id cifrado | No recolectar hasta Supabase Vault verificado en producción. Fallback: pgcrypto. |
...
### 21.1 Mapa competitivo (actualizado mayo 2026)

| Competidor | Fortaleza | Debilidad crítica vs EVA | Dato clave |
|-----------|-----------|--------------------------|------------|
| **TrueCoach** | UX limpia, coach-first, trial 14d + 90d money-back (anual) | Sin modo B2B; sin billing org; sin panel de equipo; +5% fee de pagos desde ene 2026 | $26–$137/mes (anual); theming pero NO app propia |
| **TrainHeroic** | Team management sólido, free tier, por atleta | Asistentes $9.99 c/u; sin white-label por coach; orientado a grupos | Gratis a $0.50/atleta/mes en enterprise |
| **MyPTHub** | Clientes ilimitados, 130k coaches globales | White-label app = **$157/mes extra** (no incluida); sin org dashboard nativo | ~$14/mes base, pero WL es add-on premium |
| **CoachAccountable** | Team Edition: coaches ilimitados sin costo extra | No es fitness — life coaching; sin white-label nativa | $20–$4.000/mes por clientes; Fortune 500 |
| **PT Distinction** ⚠️ | White-label incluida en Pro ($60/mes), 25 clientes | **White-label es UNA app por gym, no por coach**. Sin visión consolidada por org. | Pro $60/25c; Master $90/50c; +$2/cliente extra |
| **ABC Trainerize** | Studio/Enterprise multi-coach; PWA en app stores | Setup WL $169 one-time + fees; $248/mes mínimo Studio; no per-coach WL | $120/mes Pro 50 clientes |
| **Exercise.com** | WL completa, HIPAA/PCI, multi-coach | Desde $125/mes; sin precio publicado; cotizar siempre | Cotizar; mínimo ~$125/mes |
| **Mindbody** | Gestión gym completa, reservas, presenciales, 40k negocios | Sin coaching 1:1; $99–$499/mes + 2.99%–3.60% en pagos | Gestión de clases, no coaching |
| **Virtuagym** | Features premium, 3D exercise library | Desde ~$120/mes; sin free trial; pricing opaco; WL add-on | Sin prueba; requiere contacto ventas |
| **Glofox / ABC Fitness** | App branded incluida, precio life-locked | ~$100/mes+; solo clases grupales; sin coaching 1:1 ni nutrición | Contactar ventas; precio bloqueado |
| **Wodify** | CrossFit/box fuerte, comunidad específica | Nicho muy específico; sin white-label individual | Específico CrossFit |
...
### 21.3 Cómo responder "¿Por qué no MyPTHub?" en demos

> "MyPTHub tiene clientes ilimitados en su plan base, sí. Pero si querés tu propia app con tu marca, te cobran $157 dólares mensuales extra — y eso es UNA app del gym, no una app por coach. Con EVA cada coach tiene su app individual incluida en el precio, y además tenés el panel unificado para ver todo el equipo."

### 21.4 Moat a construir
...
4. **LATAM-first:** ningún competidor internacional tiene pricing ajustado para LATAM, soporte local ni integración MercadoPago. EVA es la única opción nativa.

### 21.5 Ideas post-MVP para evaluar
...
## 22. Análisis de precios — Posicionamiento global

> *Datos verificados mayo 2025–2026. Tipo de cambio: 1 USD = 942 CLP.*

### 22.1 Competencia internacional vs EVA individual ($29.990 CLP / ~$32 USD)

| Plataforma | Precio/mes (USD, anual) | Clientes incluidos | White-label | Panel equipo | Posición vs EVA |
|-----------|------------------------|-------------------|-------------|--------------|-----------------|
| **ABC Trainerize** | $9–$22 (~$8.500–$21k CLP) | 2–200 clientes | Solo plan caro + $169 setup | Enterprise | Más barato, pero sin WL ni organización |
| **MyPTHub** | ~$14 flat + $157 WL add-on | Ilimitados | Sí, pero add-on ~$157/mes | No nativo | Base más barata, WL 5× más cara |
| **TrueCoach** | $26–$137 ($24k–$129k CLP) | 5–50 clientes | Standard y Pro (+5% fee pagos) | No | Comparable en precio; sin panel B2B |
| **PT Distinction** | $20–$90 ($19k–$85k CLP) | 3–50 clientes | Sí (una app por gym) | No | Amenaza cercana en precio |
| **CoachAccountable** | $20–$60 ($19k–$57k CLP) | 2–30 clientes | Add-on $50/mes | Sí (unlimited coaches) | Life coaching, no fitness |
| **Virtuagym** | $29–$150+ ($27k–$141k+ CLP) | Varía | Solo premium | Sí | Sin free trial; opaco |
| **TrainHeroic** | $30–$199 ($28k–$187k CLP) | 5–1000+ atletas | Pro y superior | Sí (teams) | Grupo/atleta, no 1:1 |
| **Mindbody** | $99–$499 ($93k–$470k CLP) | Por ubicación | No prominente | Sí (staff) | Gestión presencial, no coaching |
| **Exercise.com** | $125+ | Custom | Sí (completo) | Sí | Precio alto; cotizar siempre |
| **EVA (individual)** | **$29.990 CLP (~$32 USD)** | **30 alumnos** | **Sí — por coach, no por gym** | **No (retail)** | **Único WL per-coach en este rango** |

**Conclusión:** EVA a $32 USD está en el rango medio-alto del mercado con una feature que ninguno en ese precio tiene: white-label **por coach individual**, no por gym.

### 22.2 Benchmarks mercado chileno

| Concepto | Precio CLP/mes | Contexto |
|----------|---------------|----------|
| Membresía gym promedio Santiago | $30.000–$55.000 | EVA individual < 1 membresía |
| CrossFit box premium | $55.000–$80.000 | — |
| Sesión personal trainer | $15.000–$60.000 por sesión | 2 sesiones = 1 mes de EVA |
| Software gestión gym local (GymPlus) | $35.000 (~$37 USD) | Producto diferente — no coaching |
| Sueldo promedio trainer Santiago | $425.000/mes | EVA = 7% de su ingreso |
| Salario mínimo Chile 2025 | $529.000/mes | — |

**EVA individual ($29.990) = menos del 7% del ingreso de un coach activo.** Para una herramienta que es el corazón de su negocio digital, es la zona de comodidad correcta.

### 22.3 Competencia B2B vs EVA Empresas

| Plataforma | Oferta B2B/equipo | Precio mensual | White-label por coach | Panel unificado |
|-----------|------------------|----------------|----------------------|-----------------|
| **ABC Trainerize Studio** | Multi-coach, enterprise | $248/mes+ | No — una app de gym | Sí |
| **TrainHeroic Org** | Equipos deportivos | Varía por atletas | No — branding por org | Sí |
| **CoachAccountable Team** | Coaches ilimitados | $20–$200/mes | No prominente | Sí (life coaching) |
| **Mindbody** | Por ubicación/sede | $99–$499/sede | No | Sí (staff mgmt) |
| **Exercise.com** | Multi-coach | Cotizar / $125+ | Sí (una app por gym) | Sí |
| **Glofox** | Multi-coach clases | ~$100/mes+ | Sí (una app por gym) | Sí |
| **PT Distinction** | No tiene plan org | N/A | Una app por gym | No |
| **TrueCoach** | No tiene plan org | N/A | Solo theming | No |
| **EVA Starter Gym** | Hasta 5 coaches | **$89.990 (~$95 USD)** | **Sí — por coach individual** | **Sí** |
| **EVA Pro Gym** | Hasta 10 coaches | **$159.990 (~$170 USD)** | **Sí — por coach individual** | **Sí** |
| **EVA Elite Gym** | Hasta 20 coaches | **$269.990 (~$287 USD)** | **Sí — por coach individual** | **Sí** |

**El único producto que combina white-label individual por coach + panel org unificado en América Latina es EVA.**

ABC Trainerize Studio ($248 USD) es el competidor B2B más cercano en features, pero sin WL per-coach y al doble del precio.

### 22.4 Análisis de precios por coach dentro del pack B2B

| Plan EVA | Coaches | Precio/coach (CLP) | Precio/coach (USD) | Descuento vs individual | Benchmark competencia |
|----------|---------|---------------------|---------------------|-------------------------|-----------------------|
| Starter Gym | 5 | $17.998 | ~$19 | –40% | TrueCoach sin B2B ni WL = $57/coach |
| Pro Gym | 10 | $15.999 | ~$17 | –47% | Trainerize Studio = ~$25/coach (sin WL por coach) |
| Elite Gym | 20 | $13.499 | ~$14 | –55% | TrainHeroic enterprise = $0.50/atleta (sin WL) |

**EVA ofrece WL por coach a un precio que ningún competidor puede igualar en este segmento.**

### 22.5 Posición de precio EVA individual — contexto LATAM

El estándar global SaaS para ajuste LATAM es un descuento de **30–50% vs precios USA**. Con TrueCoach Pro ($99 USD) como referencia:
- Con 50% descuento LATAM → $49.50 USD → $46.600 CLP
- EVA cobra $29.990 CLP → **36% menos que el punto de referencia ajustado**

Esto significa que EVA tiene **espacio para subir precios en el futuro** sin perder competitividad, especialmente al agregar features (AI coaching, reportes avanzados, API).

### 22.6 Escenarios de precio y recomendaciones

| Segmento | Riesgo principal | Argumento que gana | Recomendación |
|----------|-----------------|-------------------|---------------|
| Coach empezando (1–10 alumnos) | MyPTHub $14 flat | "EVA te da tu negocio digital con tu nombre desde día 1" | Mantener $29.990. Invertir en messaging de marca. |
| Coach profesional (15–30 alumnos) | Casi ninguno | $29.990 = irrelevante vs ingresos | No cambiar nada. |
| Gym con equipo | Prácticamente ninguno si el ahorro se comunica | "5 cuentas = $149.950. Nosotros = $89.990 + panel de gestión" | Mantener precios B2B actualizados. |

### 22.7 Oportunidad de expansión internacional

Si EVA expande fuera de Chile, los precios actuales son **subvalorados para mercados USD/EUR**:

| Segmento | Precio actual CLP | USD actual | Precio recomendado USA |
|----------|------------------|------------|------------------------|
| Coach individual | $29.990 | ~$32 | $39–$49 USD |
| Starter Gym (5 coaches) | $89.990 | ~$95 | $149–$189 USD |
| Pro Gym (10 coaches) | $159.990 | ~$170 | $249–$299 USD |
| Elite Gym (20 coaches) | $269.990 | ~$287 | $399–$499 USD |

**El mercado fitness app LATAM crece al 27.88% CAGR** (2025–2033), proyectado a $6.13B USD para 2033. Chile está identificado como mercado de crecimiento clave. Ningún competidor internacional tiene pricing, soporte ni integraciones locales para Chile — EVA tiene primera ventaja.

### 22.8 Tabla resumen: veredicto final de posicionamiento

| Segmento | ¿Precio OK? | Nota |
|----------|-------------|------|
| Coach individual Chile | ✅ Correcto | Competitivo para el valor que entrega; espacio para subir en 12–18 meses |
| Coach individual internacional | ✅ Subvalorado | Espacio para $39–$49 USD en expansión |
| B2B Starter Gym (5 coaches) | ✅ Correcto | $89.990 — ahorro $60K/mes + panel; ~40% descuento por coach |
| B2B Pro Gym (10 coaches) | ✅ Correcto | $159.990 — descuento justo, margen sostenible |
| B2B Elite Gym (20 coaches) | ✅ Correcto | $269.990 — WL ×20 coaches por ~$14 c/u, excelente valor |
| B2B Enterprise (21+) | ✅ Desde $400.000 | Cotizar está bien; validar con primer deal real |

**Los tres puntos más importantes:**

1. **No están baratos de manera problemática ni caros de manera que espante.** El precio individual ($32 USD) está exactamente donde debe estar: más caro que herramientas básicas, más barato que las premium, con un diferenciador único que justifica el precio.

2. **El B2B es la mejor relación precio-valor del mercado de coaching en LATAM.** No existe otro producto que combine white-label por coach dentro de un plan de equipo + panel unificado a este precio. Esa combinación es única.

3. **El único riesgo real es el segmento de coaches que empiezan**, donde MyPTHub ($14 flat) puede llamar la atención. La respuesta no puede ser bajar el precio — tiene que ser convencer al coach de que construir su marca desde el día uno es la decisión correcta.

---

*Fin del plan rev. FINAL. Todas las decisiones resueltas. Arrancar ola 1 cuando equipo apruebe.*
*Próxima revisión recomendada: cuando se complete el primer piloto o en 6 meses.*
