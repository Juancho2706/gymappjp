/**
 * @eva/plan-builder — reducer + hook del builder de programas (fuente de verdad web+mobile).
 *
 * Extraído de apps/web/src/app/coach/builder/[clientId]/hooks/usePlanBuilder.ts (E5-01).
 * Importa la resolución de áreas/agrupamiento de @eva/workout-engine (NO se duplica).
 * `arrayMove` es local (evita depender de @dnd-kit/sortable, web-only): es la MISMA función
 * pura que usa dnd-kit para índices no negativos, que es todo lo que el builder dispara.
 *
 * Sin `'use client'`: el boundary de Next lo marca el wrapper de web
 * (apps/web/.../hooks/usePlanBuilder.ts); mobile lo consume vía Metro sin directiva.
 */
...
/** Reemplazo local de @dnd-kit/sortable's arrayMove (dep web-only). Idéntico para índices ≥ 0. */
...
type BuilderAction =
    ...
    // Reemplaza los bloques de un día (builder mobile: reorden con secciones que recalcula
    // orden + section desde un drag plano). Web no lo dispara; el reducer solo lo aplica.
...
export function usePlanBuilder(initialDays: DayState[], areas: readonly WorkoutArea[] = []) {
    // Las areas llegan por props (RSC en web / fetch en mobile) y el reducer las lee via ref
    // para no recrear el estado.
    ...
    const setDays = useCallback((next: DayState[]) => {
        // Sin historial: usado para hidratar/rehacer estructura (variantes, reshape).
        dispatch({ type: 'SET_DAYS', payload: next })
    }, [])
