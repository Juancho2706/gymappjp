-- Track coach own activity (not just login, not client workout logs)
-- Adds last_active_at to coaches + debounced touch function + updated admin RPC

ALTER TABLE public.coaches
    ADD COLUMN IF NOT EXISTS last_active_at timestamptz;

-- Seed existing coaches from their last Supabase login
UPDATE public.coaches c
SET last_active_at = u.last_sign_in_at
FROM auth.users u
WHERE u.id = c.id AND u.last_sign_in_at IS NOT NULL;

-- Debounced touch: only writes if >5 min since last update (called fire-and-forget from middleware)
CREATE OR REPLACE FUNCTION public.touch_coach_activity(p_coach_id uuid)
RETURNS void
LANGUAGE sql
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.touch_coach_activity(uuid) TO authenticated;

-- Rebuild admin RPC: coach_last_login_at → coach_last_active_at (COALESCE with auth login as fallback)
...
RETURNS TABLE (
    ...
    coach_last_active_at timestamptz,
    auth_email           text,
    total_count          bigint
)
LANGUAGE sql
