-- ============================================================================
-- EVA Nutrition V2 — Enriquecimiento de read-models del alumno con media/category
-- por food_id (resuelto EN TIEMPO DE LECTURA). Cierra el TODO de NutritionFoodRow
-- (apps/web/.../c/[coach_slug]/nutrition-v2/_components/NutritionFoodRow.tsx): los
-- items prescritos y consumidos del alumno hoy solo traen name/brand/macros, asi
-- que la fila cae a un icono de categoria derivado del NOMBRE (generico y a menudo
-- equivocado, ej. "pepino" -> "otro"). El coach, en cambio, ve la ilustracion
-- especifica porque el read-model de CATALOGO si expone `media`. Esta migracion
-- lleva ese mismo `media` (mas `category`) a las superficies plan/dia del alumno.
-- ----------------------------------------------------------------------------
-- ADITIVA / forward-only / de SEGUIMIENTO: cero DROP/rename. Las tres funciones se
-- recrean con `create or replace` (firma identica => sin drop; grants/revokes
-- exactos re-asertados por si un fixer los movio). Idempotente y re-ejecutable.
--
-- NO altera snapshots congelados ni el contrato existente: los campos viejos quedan
-- INTACTOS; `media` y `category` son AGREGADOS nuevos (nullable). No se re-congela
-- nada: los snapshots ya guardan `foodId`, y media/category se resuelven por ese id
-- contra el catalogo VIGENTE (public.food_media / public.foods) en cada lectura.
--   - `media`: mismo JSON que produce private.food_catalog_v2_media_json(food_id)
--     (claves kind/bucket/objectPath/version/width/height/blurhash/...), el que ya
--     alimenta la card del coach. Ordena product_photo > eva_illustration > fallback.
--   - `category`: public.foods.category (mismas 10 categorias del icono estatico).
-- NULL-safe con food_id null (alimentos custom): media=null, category=null.
--
-- Las funciones FUENTE NO se editan (regla del rework: hallazgos sobre migraciones
-- ya aplicadas se corrigen con migracion de seguimiento, jamas editando la original).
-- Base copiada VERBATIM de 20260718150000_nutrition_portions_read_models.sql (ultima
-- definicion viva de las tres funciones — 20260714210000/20260714210500/20260716120000
-- ya no las redefinen). UNICO cambio: los tres AGREGADOS de media/category marcados
-- abajo con "-- [media/category]".
--
-- NO se aplica a prod dentro del build (BEGIN/ROLLBACK + advisors + GO CEO en la
-- fase de operacion).
-- ============================================================================

-- ── 1) nutrition_v2_intake_item_json: item consumido gana media + category. ────
--    Base = 20260718150000 (que a su vez agrego exchangeGroupCode/exchangePortions).
--    UNICO agregado nuevo: las llaves top-level 'media' y 'category', resueltas por
--    p_entry.food_id (null-safe). Resto IDENTICO (incluida la capa de porciones que
--    se sigue AGREGANDO condicionalmente con `||`).
...
-- ── 2) get_nutrition_today_v2: items PRESCRITOS del snapshot ganan media/category. ─
--    Base = 20260718150000. UNICO cambio: 'prescriptionItems' pasa de un passthrough
--    del snapshot congelado a una proyeccion que recorre los items y a cada uno le
--    adjunta 'media'/'category' resueltos por nullif(item->>'foodId','')::uuid (null-
--    safe: foodId null/ausente => media/category null). Se conserva el ORDEN via
--    `with ordinality` + `order by`. Todo lo demas (capa de porciones, consumidos,
--    unassigned, syncToken) queda IDENTICO.
...
begin
  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      ...
      -- [media/category] items prescritos del snapshot congelado: se conserva el JSON
      -- congelado (foodId incluido) y se le adjunta media/category resueltos en lectura
      -- por foodId (null-safe: foodId ausente/null/'' => media/category null). El ORDEN
      -- original del array se preserva via `with ordinality` + `order by pit.ord`. Un
      -- slot sin items => '[]' (identico al passthrough previo).
      'prescriptionItems', coalesce((
        select jsonb_agg(
          pit.item || jsonb_build_object(
            'media', private.food_catalog_v2_media_json(nullif(pit.item ->> 'foodId', '')::uuid),
            'category', (select f.category from public.foods f where f.id = nullif(pit.item ->> 'foodId', '')::uuid)
          )
          order by pit.ord
        )
        from jsonb_array_elements(coalesce(slot -> 'items', '[]'::jsonb))
             with ordinality as pit(item, ord)
      ), '[]'::jsonb),
...
-- ── 3) get_nutrition_plan_read_v2: items PRESCRITOS ganan media/category. ──────
--    Base = 20260718150000. UNICO cambio: en el jsonb de cada prescriptionItem se
--    agregan 'media'/'category' resueltos por pi.food_id (null-safe: food custom =>
--    null). La capa de porciones (exchangeTargets por franja) queda IDENTICA.
...
comment on function public.get_nutrition_plan_read_v2(uuid, date, text) is
  ...
  'Items prescritos incluyen media/category resueltos por food_id en lectura.';

-- ============================================================================
-- ROLLBACK (una pasada — NO se aplica en el build; referencia de operacion):
--
--   -- Restaurar las tres funciones SIN media/category: re-aplicar el cuerpo de
--   -- 20260718150000_nutrition_portions_read_models.sql (create or replace de
--   -- private.nutrition_v2_intake_item_json, public.get_nutrition_today_v2 y
--   -- public.get_nutrition_plan_read_v2), que es la definicion inmediatamente
--   -- anterior a esta migracion. Los campos media/category desaparecen del payload;
--   -- la UI del alumno vuelve al fallback por nombre (sin ruptura de contrato:
--   -- los schemas Zod los declaran opcionales).
-- ============================================================================
