-- Nutrition V2 hardening after the additive domain/RPC migrations.
...
create or replace function private.nutrition_v2_guard_plan_identity()
returns trigger
language plpgsql
set search_path = ''
as $$
begin
  if current_user not in ('postgres', 'service_role', 'supabase_admin') then
    if new.id is distinct from old.id
      ...
      or new.current_published_version_id is distinct from old.current_published_version_id then
      raise exception 'nutrition_v2_plan_identity_is_immutable' using errcode = '42501';
    end if;
  end if;
  return new;
end;
...
begin
  if current_user not in ('postgres', 'service_role', 'supabase_admin') then
    if new.id is distinct from old.id
      ...
      or new.publish_idempotency_key is distinct from old.publish_idempotency_key then
...
create trigger nutrition_plans_v2_guard_identity
before update on public.nutrition_plans_v2
for each row execute function private.nutrition_v2_guard_plan_identity();

create trigger nutrition_plan_versions_v2_guard_identity
before update on public.nutrition_plan_versions_v2
for each row execute function private.nutrition_v2_guard_version_identity();

revoke all on function private.nutrition_v2_guard_plan_identity() from public, anon, authenticated;
