-- Panel admin: el listado paginado contaba alumnos DEMO y ARCHIVADOS en el cupo
-- (caso Job Palacios 23-08: se agrego a si mismo como demo y quedo 1/1 en Free
-- disparando el cap-nudge). El detalle ya filtraba is_demo=false; la RPC del
-- listado no. Se filtra en el ON del LEFT JOIN para que client_count,
-- active_client_count, utilization_pct y last_activity_at consideren solo
-- alumnos reales no archivados. 16 coaches mostraban cupo inflado al 25-08.
-- NOTA: superseded por 20260826011239 (agrega demo_client_count); se conserva
-- para reproducir la historia aplicada en LIVE.
