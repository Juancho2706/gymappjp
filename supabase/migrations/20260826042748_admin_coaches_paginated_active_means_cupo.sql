-- H3-A (decision del jefe 26-08, informe vta-w4-admin §H3): `active_client_count` del
-- listado admin era el UNICO numero del panel que aun filtraba `is_active = true`.
-- Un alumno real, no archivado, pausado (is_active=false) OCUPA CUPO (predicado canonico
-- `countActiveStandaloneClients` en capacity.service.ts: is_archived=false AND
-- is_demo=false — su docblock dice literal «Usa is_archived, NO is_active») pero el
-- listado lo mostraba como no-activo: un admin podia leer «1 activo / 2 total» y creer
-- que habia lugar (misma familia del caso JPL del cap-nudge). Desde ahora
-- `active_client_count` cuenta el MISMO universo que `client_count` (cupo). La
-- redundancia visual de columnas se resuelve en UI (opcion C, pendiente de mockup).
-- Mismo tipo de retorno; CREATE OR REPLACE sin DROP, ACL intacta.
-- Validada con BEGIN … ROLLBACK contra LIVE el 2026-08-26 antes de aplicar.
...
AS $function$
    WITH base AS (
        SELECT
            ...
            COUNT(DISTINCT cl.id)::bigint                                                       AS active_client_count,
            CASE
