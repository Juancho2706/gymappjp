-- Fix: get_admin_coaches_paginated was counting workout_logs rows instead of unique clients
-- because of LEFT JOIN with workout_logs. COUNT(cl.id) multiplied client counts by their logs.
-- Changed to COUNT(DISTINCT cl.id) for accurate client_count, active_client_count and utilization_pct.
