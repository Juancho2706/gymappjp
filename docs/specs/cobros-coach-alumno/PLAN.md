...
| K4 | Cero precios en `apps/mobile/**`, incluido el literal `/mes` | regla eslint `local/no-prices-in-mobile` (`tools/eslint-rules/rules/no-prices-in-mobile.mjs`, desde `31c1f7a8`; antes `tests/mobile-no-prices.test.ts`) | `pnpm lint` (gate CI) |
