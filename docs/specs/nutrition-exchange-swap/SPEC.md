...
| **D4** | **Resuelta por el owner: ocultar.** La pill "Puede sustituir" del quick-edit (web y RN) se retira. El campo sigue en el contrato, sin lectura. | **Correccion sobre lo que decia la revision:** el checkbox del **builder ya no existia** — lo habia sacado la poda de la ola 3 (`nutrition-ui-poda`), en web (`PlanStep.tsx`) y en RN (`builder/[clientId].tsx:303-307`), con el mismo argumento. La unica superficie que seguia mintiendo eran las dos pills de quick-edit. El veto real por alimento es `exchange_group_foods.is_excluded` (H6) |
