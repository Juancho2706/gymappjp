...
- **`FoodSearch.tsx` (raiz de la carpeta, NO `_components/`) tiene 2 importadores V1** (hallazgo de la revision 2026-08-09): `nutrition-plans/_components/FoodCatalogCurationQueue.tsx:10` y `nutrition-plans/_components/recipes/StructuredRecipeDialog.tsx:16`. Borrar la carpeta entera rompe el build de V1. Decision de la revision: **`FoodSearch.tsx` NO se borra en F5** — sobrevive junto al `page.tsx` de redirect. Alternativa si el owner quiere la carpeta 100% muerta: mudarlo a `components/coach/` tocando esos 2 imports de V1.

### Dos hallazgos que cambian el alcance escrito

**H1 — Las rutas que T2.3 mandaba retirar ya no existen.** `git ls-files` no devuelve ningun `page.tsx` bajo `app/coach/meal-groups/` ni `app/coach/recipes/`. De `meal-groups` solo sobreviven `_actions/` y `_data/`, y sus **unicos consumidores son V1**. Correccion de la revision 2026-08-09: los importadores reales son **2**, no 4 — `PlanBuilder/PlanBuilder.tsx:58` (`saveMealGroup`) y `PlanBuilder/FoodSearchDrawer.tsx:18` (`listCoachMealGroups`); `FoodLibrary.tsx:220` y `NutritionHub.tsx:76` solo lo mencionan en comentarios, y a `meal-groups/_data` no lo importa nadie fuera del propio `meal-groups.actions.ts`. La conclusion no cambia (solo V1), pero F4 documenta la lista real. Las recetas viven enteras dentro de V1 (`nutrition-plans/_components/recipes/*`) mas `api/recipes/search/route.ts`.
...
5. `grep` de importadores de `app/coach/foods/_components` fuera de esa carpeta devuelve cero antes de retirar la UI (verificado 2026-08-09: ya se cumple hoy). `FoodSearch.tsx` queda explicitamente fuera del borrado (2 importadores V1).
...
2. **Los sheets importan actions de V1.** `AddFoodSheet.tsx:12` usa `saveCustomFood` de `nutrition-plans/_actions/nutrition-coach.actions`, y `ClassifyFoodSheet.tsx:21` usa `searchCoachFoodLibrary` de `nutrition-plans/_actions/food-library.actions`. **Verificado en la revision 2026-08-09: la mudanza NO viola el gate.** `check:nutrition-v2-boundaries` es una lista negra de shells V1 (`NutritionShell`, `NutritionHub`, `PlanBuilder`, `/nutrition/_components/`, `lib/flags`, …), no una prohibicion generica de importar dominio V1; precedente ya verde dentro del arbol V2: `nutrition-v2/_data/client-food-prefs.data.ts:6`. El gate se corre igual en F5 como verificacion barata. Lo que SI arrastra la mudanza: `FoodEquivalenceFields.tsx` viaja con los sheets (lo importan relativo) y el import relativo `'../_actions/food-equivalence.actions'` de `ClassifyFoodSheet` se reescribe segun la carpeta destino.
...
- **Dos lecturas distintas de `foods` (hallazgo de la revision 2026-08-09).** `FoodBrowser` lee la tabla directo via `getFoodLibrary` (sin filtro de pais ni de `verification_status`, con navegacion sin buscar, filtro mine/all server-side, categorias, orden y contador de total); el tab lee `search_food_catalog_v2` (solo `country_code` CL o null, **excluye `rejected`**, exige query de 2+ chars, orden fijo por relevancia). Consecuencia dura para F5: un alimento del coach con pais distinto o rechazado por curacion se ve hoy en `/coach/foods` y NO en el tab — "lo cree y no aparece". Antes de F5: contar en LIVE cuantos alimentos de coach caen en ese hueco, y decision del owner sobre que capacidades del navegador viejo absorbe el tab y cuales se degradan asumido.
