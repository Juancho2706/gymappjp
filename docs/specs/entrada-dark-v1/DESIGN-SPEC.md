...
- **El sweep se siembra en el VEREDICTO del gate, no en el primer paint — y el veredicto tiene dos momentos (ajuste del QA en device, 2026-08-19).** El **temprano**: sesion viva y **sin branding guardado** en AsyncStorage ⇒ no puede haber marca de coach y el veredicto no espera el select a `coaches` (si espera `getSession()`, que puede traer un refresh de token — «rapido» es el caso tipico, no un contrato). Es el caso comun (coaches EVA y sus alumnos). El **tardio**: habia branding guardado que validar contra `getCoachProfile()` y resulto no presentable. **AMBOS momentos pasan por `splashSweepSeed`**, y ahi vive el contrato duro de este frame: una semilla previa gana (jamas re-sembrar), y **si la espera ya se habia hecho lenta NO se siembra** — la firma lleva rato en pantalla con su fade por umbral y relanzar la escena completa encima era la secuencia «nativo → loader viejo → sweep → loader viejo» que el owner marco como inconsistencia. O sea: el sweep arranca temprano o no arranca; **nunca puede verse el look viejo Y DESPUES el barrido**, no porque el veredicto «llegue rapido» sino porque el guard lo impide. Gate lento = pre-Glide integro, `null` de punta a punta. Ojo al orden de creacion de promesas en `SplashGate`: `getSession()` se invoca ANTES que `getCoachProfile()` porque ambas pasan por el lock reentrante de auth-js — invertirlo encola la lectura local detras del round-trip de red y mata el veredicto temprano (hallazgo de la revision adversarial 19-08).
...
- **El handoff nativo→JS sigue siendo pixel-identico.** `hideAsync()` sale del `onLayout` de la raiz del gate, y en ese instante todavia no hay semilla: la figura esta centrada a 150 pt donde la dejo el nativo. El costo asumido llega despues, en el veredicto: la figura **salta** fuera de cuadro para entrar barriendo. Ese blink es la decision «sweep fiel» — el gesto de marca completo vale mas que un empalme invisible, y arrancar el barrido desde donde estaba la figura convierte el diseno en otra cosa. Los 75 ms de hold de la escena son su unico colchon. Con el veredicto **temprano** el salto suele llegar enseguida del `hideAsync()` y fundirse con el handoff (medicion de device, no invariante: `hideAsync` y el veredicto son asincronias independientes).
- **El reloj es tiempo real, no un progreso local.** Con el veredicto temprano el gate retiene la escena hasta que la red define el destino; con el tardio suelta el control pocos ms despues de sembrar. En ambos casos la cola de la escena la pinta `DashboardSplashOverlay`: hereda `sweepStartedAt` por `SplashHandoff` y calcula su propio `elapsed`. Toda la coreografia es funcion pura de `t`, por eso el relevo cae en el frame exacto. **El handoff sin semilla (`null`) se hereda TAL CUAL, sin fallback `?? Date.now()`**: `null` es informacion (rama branded, o veredicto lento suprimido a proposito) y la figura sin semilla no queda escondida — `SplashEvaMark` ni aplica el estilo animado y pinta la replica estatica. Un fallback aca relanzaria post-navegacion la escena suprimida.
...
1. Cold start sin sesion, Android gama baja **en release build**: nativo → replica sin flash; figura en la **misma posicion** en ambos (centro de pantalla, 150 pt). El «Glide» **no** entra por aca: sin sesion el gate se va al selector y el frame 01 tiene que verse igual que antes de la coreografia — figura quieta, sin estelas, firma con su fade por umbral. Mismo criterio en el **retorno branded**: si aparece una sola estela cian sobre el crossfade a la marca del coach, es un bug (§3.1, excepcion Glide).
   - **1b. Sweep «Glide», solo en sesion + marca EVA** (cuenta sin white-label, o coach Free): tras el veredicto la figura salta fuera de cuadro y entra barriendo. Verificar (a) que el barrido **no se corta** en el relevo `SplashGate` → `DashboardSplashOverlay` —tiene que seguir de largo, no volver a empezar—, (b) que en un dispositivo de **320 pt** la figura sale **entera**, esquina inclinada incluida, durante el hold, (c) que las 3 estelas se apagan antes de los 750 ms y no reviven, (d) que **jamas se vea la firma con su fade viejo ANTES del barrido** — en el caso tipico sin branding guardado el sweep arranca antes del umbral de 600 ms; si la espera igual se hizo lenta (AsyncStorage frio, refresh de token, red estrangulada) el guard de `splashSweepSeed` suprime el sweep y queda el pre-Glide integro, en CUALQUIERA de las dos ramas del veredicto—, y (e) que con veredicto **lento** no haya relanzamiento de la escena ni en el gate ni en el overlay post-navegacion (el handoff hereda `null` tal cual, sin fallback).
