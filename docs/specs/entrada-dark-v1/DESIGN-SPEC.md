...
**Historial «Glide» (retirada, 2026-08-19).** La coreografia de barrido —la figura entrando desde la izquierda con estelas, disenada el 18-08 e implementada en `837b48b9`/`592f3714`— fue **RETIRADA por decision del dueno tras verla en device**: la quietud vuelve a ser la regla de este frame, sin excepciones. Con ella se retiro tambien la deuda que la hacia inevitable de ver: la replica JS pintaba la figura a **150 pt** mientras el splash nativo usa **`imageWidth: 180`**, y centraba el stack figura+firma en vez de la figura — el logo se achicaba ~20% y saltaba hacia arriba en cada handoff. El contrato vigente:

- **La figura mide 180 pt** (`SPLASH_FIGURE_SIZE` = el `imageWidth` de `app.json`, la razon de ser del numero es la paridad con el nativo) y vive en el **centro real de la pantalla** (`splashFigureCenterY = h/2`), que es donde el sistema operativo centra su imagen. Cambiar cualquiera de los dos lados sin el otro reintroduce el salto.
- **La figura no anima jamas, en ninguna rama.** Es la unica hija en flujo del contenedor centrado; la firma (hairline + wordmark) cuelga DEBAJO en un slot **absoluto** (top 50% + media figura + 16 pt) — aparecer o desaparecer no la mueve un solo punto.
- La vida del arranque pasa **alrededor** de la figura: la firma con su fade (§4 S2), el halo que respira (§4 S3), el morphbar (§4 S4) y —en el retorno branded— el crossfade de 260 ms a la marca del coach (§2.3), anclado al MISMO centro para que la luz no salte de canal.
- Los loaders DENTRO de la app (post-arranque) no son de esta familia: siguen el white-label del coach o el estandar EVA, y este frame no los gobierna.

**Layout** (reescrito 2026-08-19 con la retirada del Glide — el stack centrado murio)
- **La FIGURA es lo unico que participa del centrado**: unica hija en flujo de un contenedor absoluteFill center/center ⇒ su centro optico cae en el **centro real de la PANTALLA** (`splashFigureCenterY = h/2`), que es donde el splash nativo centra su imagen. NO volver al stack figura+firma centrado: corria la figura ~21 pt hacia arriba del nativo y era la mitad del salto del QA 19-08.
  1. **Figura EVA** — `apps/mobile/assets/eva-icon.png`, `width: 180`, `height: 162` (aspect 585:526 = 1.1122; 180 = `imageWidth` del splash nativo, guard de paridad en `tests/mobile-splash-geometry.test.ts`), `contentFit: 'contain'`, `tintColor` NO se aplica (el PNG ya es blanco puro).
  2. **Firma en slot ABSOLUTO** (no empuja a la figura): `position:'absolute'`, `top:'50%'`, `left/right: 0`, `marginTop: 162/2 + 16 = 97` (`SPLASH_SIGNATURE_SLOT_OFFSET`), `alignItems:'center'`. Adentro: **hairline** `width: 34`, `height: 1`, `backgroundColor: 'rgba(255,255,255,.16)'`, `marginBottom: 13` (el aire superior de 16 vive en el slot) y **wordmark "EVA"** `fontSize: 11`, Hanken `800`, `letterSpacing: 4.84` (`.44em`), `paddingLeft: 4.84` (compensa el tracking colgante), `textTransform: 'uppercase'`, `color: 'rgba(244,246,248,.55)'`.
- **Halo**: fuente puntual §1.4. Canal EVA anclado a `splashFigureCenterY` (h/2); canal COACH anclado a `splashCoachSourceCenterY` (h/2 − 31, el centro optico del tile — con el ancla comun la luz quedaba debajo del logo tras el crossfade).
...
**Capa A — identidad EVA**: identica al frame 01 (figura 180x162 + hairline 34x1 + wordmark `EVA`).
...
**Hallazgo adicional (accionable)**: el alpha de `eva-icon.png` es practicamente **binario** — de 77.059 muestras, solo **29** tienen alpha intermedio. El PNG no trae antialiasing en el canal alpha. Ojo con la cuenta EN PIXELES FISICOS: a 180 pt el asset de 585 px se pinta a 360 px (@2x, downscale 1.63x), **540 px (@3x, 1.08x — practicamente tamano nativo)** y 720 px (xxxhdpi Android, AMPLIACION 1.23x). No es una regresion contra el splash del sistema —el config plugin genera el drawable/asset nativo a `180 × densidad` desde el MISMO fuente, o sea que el OS ya hace el mismo remuestreo— pero el margen de suavizado que existia a 150 pt se redujo: **QA en device 3x y xxxhdpi debe mirar el borde de la silueta** (item §7.1). La regla general se mantiene: no usar este asset ampliado en superficies NUEVAS, y en el splash **nativo** verificar en device que el remuestreo de Android no produzca bordes dentados a `imageWidth` bajos.
...
["expo-splash-screen", {
  ...
  "imageWidth": 180,                   // FUENTE DE VERDAD del tamano: la replica JS copia ESTE numero (§3.1, decision del dueno 2026-08-19 — el 150 del mockup original nunca se aplico al nativo y la replica se alineo al reves)
  "resizeMode": "contain",
...
**Replica JS del splash**: `expo-image` con `source={require('../assets/eva-icon.png')}`, `contentFit="contain"`, **`transition={0}`** (la imagen debe estar en el primer frame, no aparecer con fade), `cachePolicy="memory-disk"`, `priority="high"`. `width: 180`, `height: 162`. **Centrada en el centro de la PANTALLA, no del body** — ver §3.1 (el centro optico de la figura ES h/2).
...
| 3 | Figura EVA como **data URI base64** dentro de un `<symbol>` + `<use>` | Auto-contencion del HTML (1 solo binario, cero CDNs) | `require('../assets/eva-icon.png')` con `expo-image`. Un solo asset, 4 tamanos (180 / 30 / 22 / 22) |
...
1. Cold start sin sesion, Android gama baja **en release build**: nativo → replica sin flash; figura en la **misma posicion Y el mismo tamano** en ambos (centro real de pantalla, **180 pt** = `imageWidth`). La figura esta QUIETA en todas las ramas (el «Glide» fue retirado el 19-08, §3.1): cualquier movimiento, escala o estela sobre la figura es un bug.
   - **1b. Handoff invisible + firma sin empujar**: verificar que el paso nativo → JS no produce NINGUN salto de tamano ni de posicion de la figura (es la misma imagen en el mismo pixel), y que la aparicion de la firma EVA (fade por veredicto temprano ~150 ms sin branding guardado, o por umbral de 600 ms) **no mueve la figura** — cuelga absoluta debajo. En el **retorno branded**, el crossfade a la marca del coach cruza sobre la figura quieta, anclado al mismo centro.
