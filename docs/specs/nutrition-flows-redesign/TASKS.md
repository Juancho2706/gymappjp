...
- [~] Cierre O1 (2026-08-06/07):
  ...
  - [~] QA device fisico Android (2026-08-06 noche, celular del owner via Metro+USB — dev client INSTALADO y funcionando):
    ...
    - Login alumna OK con datos reales; hallazgos de la ronda: (A) items prescritos consumidos SIN lapiz/retirar en RN (web si tiene — gap NUT-009) → ARREGLADO: chip Registrado + lapiz (gated por canAdjustPrescribedQuantity) + retirar (nunca se esconde); (B) "Otro motivo" del sheet de correccion quedaba tras el teclado → ARREGLADO: snapPoints 85%/78%. tsc mobile verde.
    - PENDIENTE re-verificar ambos fixes en el device + puntos 1 (banda/arco), 4 (pill precargada), 5 (Plan podado), 6 (historial), 7 (fuera-del-plan invisible) + veredicto owner. Runbook celular en memoria android-emulator-vision-setup (conectar USB → Metro → adb reverse → deep link eva://expo-development-client).
  - [ ] Checklist de preservacion completa (SPEC) — pasada parcial implicita en el QA de arriba, formal pendiente
