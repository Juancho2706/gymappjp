...
> **Enmienda 11-09 a R2 (decisión del owner tras el QA en device, F1).** En fuerza por tiempo **las repeticiones no se quitan**: el hero muestra **KG · REPS · SEG** en RN y web (pantalla sola y superserie) y el keypad pide KG → REPS → SEG. REPS es **opcional**: `reps_done = int(values.reps)` si es un entero > 0, si no `NULL` (nunca `0`, CA-25 sigue). Con reps > 0 la serie **sí** cuenta para récords (M2 filtra `reps_done > 0`) y `detectPR`: es lo pedido («no sabemos si el alumno quiere colocar sus reps también»). `formatStrengthTimeSetLine` ⇒ `«45 kg × 5 · 30 s»` con reps, `«10 kg × 30 s»` sin reps. CA-98 pasa de «`reps_done` se elimina del FormData» a «se elimina si está vacío». El Playwright W6.10 no tipea reps ⇒ su assert `reps_done IS NULL` sigue válido.
