...
«Más» (`ba074a92`); B2 y B5 `668dfd00`; B3 `7d9f1710` (seis botones sin «no incluido en tu plan
actual»); B7 (tokens 600) y B10 (lint limpio) `ba074a92`; B9 `3f0d9764`; B6 y B8 en el mismo tren.
**QA del owner en device VERDE 02-09** (logout solo en «Más», ficha/builder de nutrición sin el copy
de plan). B4 = NO (alcanza la lupa en Inicio); B12 = dejar; B13 = decisión tomada; B14 bloqueado por
...
### Ola de orden W1→W4 «La casa ordenada»

**Ola de orden W1→W4 «La casa ordenada» — CERRADA (`status: done`, 2026-09-01; [SDD](../ola-de-orden/TASKS.md) § «Cierre de la ola»):** en producción web (`master` = `rnmobiledenuevo` = `9991d42b`, deploy `dpl_A656yxZEoc6NWB9WMtq55fN9htbC` READY) y OTA 1.1.2 canal `production` (tren 1 android `6cd2d29d` / ios `8548c0c0`; tren 2 android `27e920aa` / ios `1677cf76`), QA del owner VERDE en web (preview) y en celular (ronda 2 + QA final del tren 2). Mockups aprobados `9801fec7` (W1: 1A 2A 3A 4A) y `bff90120` (W2→W4: 1A 2A 3A 4A 5A 6A 7C 8A 9A 10A 11B). Qué quedó: apagar un dominio cierra la ruta de verdad (gates web con redirect + `domain_off` in-page, `useDomainGuard` + `DomainOffNotice` en 7 pantallas RN, ficha por dominio, `FEATURE_PREFS_ENABLED` retirado); sidebar web en Principal / Tu trabajo / Gestión; barra RN y cápsula PWA «Inicio · Alumnos · 2 dominios · Más» con **orden editable** desde Funciones (fila `_nav` de `coach_feature_prefs`, sin migración) y «Cerrar sesión» en «Más»; pantalla única «Funciones» web + RN (Módulos + Mi panel + Herramientas; 4 redirects legacy; switch por dominio del pool para gestores de team); todo-incluido sin candados ni «Pro/Base» (`ModuleOffNotice` = kill-switch de operador), tab «Check-ins» borrada, lupa del buscador en el header de Inicio RN; **KPI del dashboard con deltas reales** (7C fase 1 en la query + fase 2: tabla `coach_kpi_snapshots`, migración `20260901230436`, cron 04:30Z, 91 filas sembradas ⇒ primer delta de «En riesgo» el 08-09). Suite completa al cierre verde (lint · typecheck · vitest 8366 · tsc mobile · tokens · boundaries · docs · expo export). **Backlog heredado: 16 ítems (B1–B16) en TASKS § «Backlog heredado», ninguno bloqueante** — los visibles: logout duplicado Más/Mi cuenta, párrafo «plan pago» de Suscripción, CTAs de plan en nutrition-v2 RN, reach global del buscador, demolición de Enterprise (ola propia). Regla del owner anotada: **ENTERPRISE eliminado** (no tocar; demoler a futuro). Hotfix ajeno a la ola en el mismo tren: `850d85a9` (el gate del proxy exime `/coach/subscription/flow-processing`; la reactivación por Flow de un coach bloqueado completaba en loop) — seguimiento en [MANUAL_TASKS § BILL-01](../../operations/MANUAL_TASKS.md).
