...
| V0.4 | Sonnet | `apps/web/src/app/dev-harness/nutrition-editor/page.tsx` (nuevo, patrón de `dev-harness/nutrition-tabs`) + script `apps/web/scripts/` espejo del existente del harness | Página sin auth que monta `QuickEditPlanView` en modo editor con draft semilla del PLAN §Verificación (usar `draftToEditState`); toggle light/dark y query `?w=` para anchos. Script Playwright headless: capturas 390/768/1024/1280/1536 ×2 temas + humo del popover. | `node` script sale 0 con capturas en `/tmp`; página no aparece en build de prod (mismo guard que el harness existente). |
