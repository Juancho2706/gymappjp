...
- [x] T3.7 - ⚠️ refund/chargeback Flow ✅ RESUELTO COMO PROCEDIMIENTO 2026-07-09: los refunds del PANEL de Flow NO emiten webhook (el callback solo existe para refund/create nuestro, y su shape no trae linkage al coach) → cancelar-la-sub-por-codigo es INALCANZABLE hoy. Resolucion: (1) procedimiento manual en RUNBOOK §'REFUND de una sub Flow' (cancelar sub PRIMERO, refund despues, marcar coach, auditar); (2) backstop automatico = flow-reconcile alerta flow_status_divergence en <24h si alguien refundea sin cancelar; (3) cablear refund/create + urlCallBack cuando exista UI de refunds (el codigo normalizeFlowRefund ya esta listo para ese dia)
