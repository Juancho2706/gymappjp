...
Implementado (presentación pura; engines/cola offline/optimistic/server actions intactos):

- **[P1 CONFIRMED] Timers flotantes re-skineados al palette inmersivo oscuro** — `RestTimer.tsx`, `HoldTimer.tsx`, `Stopwatch.tsx`, `IntervalTimer.tsx`. Contenedor `bg-card/95 border-border/60` → `bg-[var(--ink-900)]/95 border-[var(--border-inverse)]`; texto `text-foreground`/`text-muted-foreground` → `text-on-dark`/`text-on-dark-muted`; anillo/flash de descanso+hold `var(--theme-primary)`/`bg-emerald-500/10` → `var(--ember-500)`/`bg-[var(--ember-500)]/15` (ember = estado activo del kit); track del anillo `text-muted/30` → `text-white/10`; botones ghost → `text-on-dark-muted hover:text-on-dark hover:bg-white/10`; chips de vuelta del Stopwatch `bg-muted` → `bg-white/[0.06]`. IntervalTimer además: PHASE_COLOR raw (`sky/red/emerald-500`) → tokens DS (`sport-300`/`ember-300`/`aqua-500`, kill del sky-500 white-label), barra de fase = ember en `work` y `theme-primary` en el resto, `finished` `emerald-500` → `--success-500`, wakeLock `amber-500` → `--warning-500`.
- **[P1→P2 DOWNGRADED] Callout "Nota del coach" ámbar → sport/azul** — `WorkoutExecutionClient.tsx` loose (~1224) y superserie (~646): `border-amber-400/30 bg-amber-400/10` + `text-amber-300`/`text-amber-100/90` → `border-primary/20 bg-primary/[0.10]` + `text-[var(--sport-300)]` + `text-on-dark/90` (1:1 con kit §211-216).
- **[P2] Chip de tipo con icono tipado + label coloreado** — `RUT_TYPE_META` gana campo `icon` (Dumbbell/HeartPulse/Move/GitCommit) + helper `TypeGlyph`. Loose type-chip (~1098): punto genérico → icono tipado + label `style={{color: meta.color}}` (kit §183). Chip del miembro de superserie (~587): punto genérico → icono tipado coloreado (label muscular se deja neutro por ser chip de músculo, no de tipo).
- **[P2] Botones atrás/tuerca del header = cuadros 40px `bg-white/[0.08]` persistente** — back (~934) de icono pelado a `h-10 w-10 rounded-control bg-white/[0.08]`; tuerca (~965) fondo persistente en base (antes solo en hover).
- **[P2] Tipografía** — nombre loose (~1110) `text-[19px]` → `text-[22px]` (kit 23; 22 por densidad de la lista apilada no-carousel, dentro del rango 21-23 del informe); labels de objetivo `tracking-wide` → `tracking-[0.06em]` (replace-all del patrón `text-[9.5px] font-bold uppercase`); card de ejercicio `p-3` → `p-4` (subida hacia el 20px del kit sin duplicar padding contra el wrapper de grupo `p-4`).

No tocado: estructura exec + superseries intercaladas, LogSetForm, grids de objetivos, barra de progreso, botón play. Nombre de miembro de superserie se deja en 17px (subordinado).
