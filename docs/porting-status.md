...
- Tanda directorio aplicada/verificada: `load()` ya no deja loader/refresh bloqueado ante error; estado inicial tiene retry; dismiss diario usa Santiago; filtros Riesgo y Programa son independientes y combinables como web; chips incluyen programa y búsqueda; banner vencidos activa Programa; borrado exige escribir el nombre exacto con guarda anti doble-tap; reset muestra la contraseña temporal persistente y permite copiarla. Typecheck, tokens 86/86 y export Android verdes.
- Tanda ficha Resumen/Progreso/Plan aplicada: hero usa `deriveClientStatus` compartido y ya escala a Urgente; fechas/semana del plan usan Santiago; módulo body-composition OFF muestra estado+CTA en vez de spinner infinito; Overview corrige KPI de adherencia de entreno, acepta fotos front/side/back y recupera CTA final Editar plan; Progreso conserva card de peso con un solo dato; umbrales de energía alineados a web 7/4. Test `profile-analytics` 9/9 y gates completos verdes.
- Tanda datos Entreno/Programa aplicada: el select móvil ahora incluye `program_phases`, `superset_group`, thumbnail, snapshot de meta, nota, RIR, sustitución y nombre de plan. Programa muestra vigencia, barra/leyenda de fases, superserie y fallback thumbnail; detalle de sesión muestra banner de sustitución, meta, color vs objetivo, RIR y notas. Sin migración ni endpoint nuevo. Typecheck, tokens 86/86 y export Android verdes.
- Tanda chrome/búsqueda global verificada y cerrada: programas abren el `programId`/plantilla concretos; ejercicios consumen `?q=` al recuperar foco; Enter/Buscar abre el primer resultado; debounce/spinner, focus-ring, estados pressed, thumbnails, copy, accesibilidad y métricas tipográficas se alinearon al web. Quedan sólo adaptaciones touch justificadas (modal/Cancelar, target 44 px, sin hover/flechas). Reporte: `docs/rn-port/specs/seccion-3/verify-fix/chrome-global-search.md`.
- Tanda chrome/workspace switcher verificada y cerrada: `nativeModal`, copies, filas/tokens activos, iconografía, labels con rol, subtítulos de tipo, trailing “Actual” y caret del avatar multi-workspace coinciden con web móvil. La persistencia local sin spinner queda como adaptación nativa porque no hay espera de red. Reporte: `docs/rn-port/specs/seccion-3/verify-fix/chrome-workspace-switcher.md`.
- Tanda chrome/tabbar verificada y cerrada a nivel código/spec: eliminados el slot y sheet RN “Más”; ahora deriva hasta cinco accesos directos en el orden web, contempla Equipo en workspace team, usa `sport-600`/`ink-400`, renderiza Reactivar como tab estándar y replica minimización on-scroll (umbral 6 px, y>80, insets 14→72, labels fade) en todas las regiones verticales de las tabs del coach. `/coach/team` y `/coach/reactivate` viven dentro de `(tabs)` para conservar la cápsula; gate de suscripción y deep links secundarios preservados. Falta smoke visual device light/dark + EVA/custom. Reporte: `docs/rn-port/specs/seccion-3/verify-fix/chrome-tabbar-layout.md`.
- Tanda dashboard/shell verificada y cerrada a nivel código/spec: refetch por foco, refresh y error recuperable; umbral Teams 80; Insights/news/workspace cableados al destino web; orden móvil preservado. El wrapper ahora replica gutter 20, offset superior safe+16 y reserva inferior safe+88; copy de error verbatim. Reporte: `docs/rn-port/specs/seccion-3/verify-fix/dashboard-shell.md`.
- Tanda A dashboard/sections aplicada (unidad aún abierta): header alineado al bloque PWA realmente montado; FAB reducido a Crear alumno/Importar/Programa con copy, rutas y posición web; billing banners usan ramps semánticas y acciones verbatim; PulseHero recupera count-up 820 ms y delta/metric typography exactos. Pendiente ClientStatsSheet, onboarding/modales y barrido final. Reporte vivo: `docs/rn-port/specs/seccion-3/verify-fix/dashboard-sections.md`.
- Tanda B dashboard/sections aplicada (unidad aún abierta): ClientStatsSheet ahora es bottom `Sheet nativeModal` 88% con handle/header/tabs/filas/umbrales tipográficos y cromáticos del web; se retiró telemetría extra. El mapping mantiene datasets independientes de adherencia/nutrición y copy `·`, sin ceros inventados. Pendiente onboarding/modales y barrido final. Reporte vivo: `docs/rn-port/specs/seccion-3/verify-fix/dashboard-sections.md`.
...
Próxima unidad Sección 3: `directory-sheets`; después `directory-create-import`
y las cuatro unidades de ficha aún sin reporte final.
