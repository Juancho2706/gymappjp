...
| Alimentos: buscar/orden/scope, categorías, nuevo alimento (medida casera), borrar+undo | `FoodLibrary` (V1) + tab Alimentos del hub V2 (`FoodCatalogBrowser`, `AddFoodSheet`, `ClassifyFoodFlow` en `nutrition-v2/`) · mobile `foods` — `/coach/foods` es redirect desde T2.3 F5 (`1eaea68c`); orden/categorías/borrar+undo quedaron degradados asumido (decision owner 2026-08-09) | coach-04 §Tab Alimentos | ✅ mapeada |
| Recetas (sistema NUEVO base): nueva (imagen WebP), compartir, editar, eliminar | `nutrition-plans/_components/recipes/*` [web] · mobile tab Recetas del hub V1 (`lib/recipes-coach.ts`, CRUD + imagen) | coach-04 §Tab Recetas | ✅ mapeada (corregido 2026-07-29: mobile SÍ la tiene) |
...
| **Nutrición Pro (client-plan):** ExchangeModePanel (gramos↔porciones), ExchangeTargetsEditor, variantes de día, PDF branded | `PlanBuilder/ExchangeModePanel`, `ExchangeTargetsEditor`, `exchange.actions` [web] · mobile `nutrition-builder` (V1: intercambios, variantes, export PDF) + builder V2 (porciones/variantes/drafts, ola 4B) | coach-04 §ExchangeModePanel (gate `nutrition_exchanges`) | ✅ mapeada (corregido 2026-07-29: mobile SÍ la tiene) |
| **Grupos de Alimentos** (saved_meals: crear grupo, items+cantidades, totales) | dentro del creador de planes: "Guardar como grupo" (`PlanBuilder`) + pestaña "Grupos" del `FoodSearchDrawer`, sobre `meal-groups/_actions` + `_data` [web] | coach-04 sin tab "Grupos": el gesto vive DENTRO del builder | ✅ resuelta 2026-08-05 (ruta `/coach/meal-groups` y su biblioteca borradas; las actions siguen vivas) |
| Recetas sistema LEGACY (`/coach/recipes` Edamam) | — borrado 2026-08-05 (la ruta ya solo redirigía) | coach-04 (sistema viejo; el sistema nuevo es el tab "Recetas" del hub) | ✅ mapeada (deprecación ejecutada) |
...
| **Cardio (módulo):** calculadora zonas FC, calculadora pace, galería intervalos, editor perfil alumno | `cardio/CardioToolsClient`, `cardio/[clientId]/CardioProfileForm` [web] · mobile `coach/cardio/*` (E6, zonas/pace/plantillas/perfil) | coach-05 (`/coach/cardio`, gate `cardio`) | ✅ mapeada (corregido 2026-07-29: mobile SÍ la tiene) |
| **Movimiento (módulo):** hub semáforo, reporte+evolución+historial+borrar, wizard 8 pasos, print branded | `movement/*` [web] · mobile `coach/movement/*` (E6, hub+wizard+reporte; export PDF agregado 2026-07-29 vía `movement-report-pdf.ts`, sin bitácora `pdf_generate` — delta anotado) | coach-06 (`/coach/movement`, gate `movement_assessment`) | ✅ mapeada (corregido 2026-07-29: mobile SÍ la tiene) |
...
| **Equipo (Teams):** hero cupos/alumnos/módulos, TeamBrandStudio, TeamMembersManager (agregar/rol/co-gestor/transferir/sacar), TeamShareLink | `team/*` [web] · mobile `coach/settings/team.tsx` (subset E7: agregar/sacar/co-gestor + nombre/color; web-only: transferir propiedad, matriz de permisos por miembro, especialidad, logo/loader) | coach-10 (`/coach/team`, solo coach_team) | ✅ mapeada (corregido 2026-07-29: mobile tiene subset; resto = deuda acotada) |
...
| **Mobile · tab Check-ins** (visor read-only de check-ins entre alumnos) | ~~mobile `coach/(tabs)/check-ins.tsx`~~ — retirado en W4.6a (Ola de orden, 2026-09-01, decisión 8A del owner) | coach-01 ActivityFeed (`MobileNovedades`) + coach-02 ficha (check-in snapshot + marcar revisado en `OverviewTab`, historial en `ProgresoTab`) | ✅ demolida (ver ítem 5 de dudas) |
...
2. **✅ RESUELTA 2026-08-05 — Grupos de Alimentos / saved_meals (meal-groups)** — se confirmó la **deprecación de la superficie**, no de la función: la ruta `/coach/meal-groups` y su biblioteca (`MealGroupLibraryClient`, `MealGroupModal`, `page`, `loading`) se borraron y los enlaces que apuntaban ahí también. Los grupos se siguen creando e insertando DENTRO del creador de planes ("Guardar como grupo" + pestaña "Grupos" del `FoodSearchDrawer`), así que `meal-groups/_actions` y `_data` quedan vivos. No hay 5ª tab.
...
5. **✅ Tab "Check-ins" del coach (mobile) — retirado en W4.6a (2026-09-01, decisión 8A del owner: se acepta la distribución, sin inbox)**. Estaba huérfano: 0 `router.push`/`href` apuntaban a él fuera de su propio archivo y `MOBILE_TAB_KEYS` (`apps/mobile/components/coach/coach-tab-keys.ts`) nunca lo incluyó. La revisión de check-ins queda repartida como ya lo hacía: dashboard `MobileNovedades`/ActivityFeed (`apps/mobile/components/coach/CoachDashboardSections.tsx`) + snapshot y "marcar revisado" en la ficha del alumno (`clientDetail/OverviewTab.tsx`) + historial en `clientDetail/ProgresoTab.tsx`.
...
> **Nota — deuda de paridad mobile (corregida 2026-07-29 contra código, rama `rnmobiledenuevo`):** la lista anterior de esta nota quedó obsoleta — la mayoría YA está construida en mobile: módulos Cardio/Movimiento/Composición (`coach/cardio|movement|bodycomp`), Nutrición Pro (intercambios/variantes/PDF en `nutrition-builder` V1 + porciones/drafts en builder V2), Equipo (subset), import XLSX/CSV (`ImportClientsForm`), Recetas (tab V1), settings Módulos/Funciones/Áreas, y del lado alumno: off-plan, lista de compras, recap semanal, swaps/intercambios, quick-log de peso (todo en la tab nutrición V1 + superficie V2). **Deuda real vigente:** flujos de dinero = link-out por diseño (money-safety/IAP, no es deuda); eliminar cuenta vía mailto (web tiene DangerZone tipada); Soporte mobile = mailto+FAQ (el diseño pide formulario); branding avanzado subset (sin rueda libre ni preview de fuentes — decisión); objetivos PRO bodycomp en sidebar del builder V1 (verificar); restricciones-alimentarias/nutrient-targets en ficha (pendiente portar a V2 — ver retiro V1); transferir propiedad/matriz de permisos del Equipo; push opt-in del alumno (la infra de envío `sendPushToClient` no tiene disparadores HOY en ninguna plataforma — hallazgo 2026-07-29). El estado canónico de paridad vive en `MOBILE_PARITY.md`; esta matriz es referencia del rediseño.

---

## Reubicaciones notables (movimientos de IA deliberados del diseño)

1. **Navbar coach 9 → 5 tabs.** Hoy mobile tiene 9 tabs (`coach/(tabs)/_layout.tsx`: Dashboard, Alumnos, Programas, Ejercicios, Nutrición, Mi Marca, Suscripción, Soporte, Mi cuenta; el tab Check-ins se retiró en W4.6a, 2026-09-01). El diseño los colapsa a **Inicio · Alumnos · Programas · Nutrición · Opciones** + un menú **"Más"** que agrupa Opciones/Soporte/Equipo/módulos. (`readme.md` IA + `CoachSidebar` split-nav.)
...
8. **Recetas: del menú/sistema viejo al tab del hub.** `/coach/recipes` (Edamam legacy) **se borró el 2026-08-05** (ya solo redirigía a `/coach/foods`); el sistema nuevo de recetas vive como **tab "Recetas"** del hub de Nutrición (coach-04).
