...
**Hecho + en prod + testeado:** team pool (tablas/RLS optimizada/governance/áreas pipeline), guards de app (read/list/write workout+nutrición), entitlements + Settings>Módulos, **app del alumno de pool `/t/[team_slug]`** (proxy + RPC + login + branding), **consentimiento Ley 21.719** (gate + grant + bitácora `team_access_logs` + bucket privado salud), **gestión de miembros `/coach/team`** (crear/vincular/sacar/co-gestor/transferir, con hardening post review), **CEO Teams panel `/admin/teams`** (provisión por UI: crear team + owner + seat_limit + módulos). Commits `7443ad5`, `63c0f53`, `c40dac2` (+ CEO panel) en `feat/movida-platform`. **Loop de provisión completo:** CEO crea team+owner → owner suma coaches → alumnos entran por `/t` con consent.
**Próximo (en orden):** (1) **A.bis2** flujos de entrada al pool (sembrar `client_memberships` scope `team` en resolve-invite/import/join). (2) **C Settings hub** (marca team vs personal). (3) **E awareness** (`last_edited_by` + badge). (4) **Builder áreas UI** (specced en `specs/movida-areas/`). Luego Plan 2 (entrenamiento polimórfico + cardio).
