...
|---|---|---|
| Web/PWA | Pricing v3 productivo: Free = 1 alumno + white-label + sello «Hecho con EVA»; Pro 25 sin sello. **Deploy vigente 10-09 19:03Z: `master` = `rnmobiledenuevo` = `27132cb9`, `dpl_EjxDinR2…` READY** — «Señales honestas para el coach» ([SDD](../specs/senales-honestas-coach/SPEC.md); QA del owner VERDE ⇒ `done`). **En código local, sin push (10-09 ~20:30Z): tren «Cuenta atrás en pantalla»** ([SDD](../specs/cuenta-atras-en-pantalla/SPEC.md); migraciones M1/M2 ya en LIVE desde W0) — espera el OK del owner para push → deploy → OTA. Deploys anteriores en el [historial](../archive/current-historial-2026-09.md). | [Runbook](../operations/RUNBOOK.md) · [spec](../specs/pricing-v3/SPEC.md) |
| App nativa (RN) | 1.1.2 es el piso OTA; canal `production` recibe android e ios por separado sobre el mismo commit. **OTA vigente 10-09 19:08Z** desde `master` @`27132cb9` (android `a95c9e11` / ios `bd329833`): «Señales honestas para el coach», QA del owner Q1–Q24 VERDE ⇒ SDD `done`. Antes el mismo día: 04:07Z @`d5d7d188` (fix P1 marca cruzada + «Adherencia —») y 02:27Z @`95a1d39a` («Porciones a la chilena»). **Próxima OTA: «Cuenta atrás en pantalla»** (misma 1.1.2, después del deploy web; pendiente de OK del owner). OTAs anteriores y QA de device en el [historial](../archive/current-historial-2026-09.md). | [Mobile parity](MOBILE_PARITY.md) · [OTA](../operations/MOBILE_RELEASES_OTA.md) |
...
1. **Tren «Cuenta atrás en pantalla» (feedback Movens 10-09 + preferencia D5 «Pasar solo al descanso») — EN CÓDIGO LOCAL, sin push (10-09 ~20:30Z)** ([SDD](../specs/cuenta-atras-en-pantalla/SPEC.md) · [tareas](../specs/cuenta-atras-en-pantalla/TASKS.md) · [datos y QA](../specs/cuenta-atras-en-pantalla/DATA-TESTING.md)): W0–W5 completos en `rnmobiledenuevo` (`1b93d58e`): reloj bajo el video también en superserie (RN y web), guardado solo a 0 con `metadata.hold_source`, fuerza por tiempo «Reps | Segundos», preferencia por alumno con modal de primera vez, CTA «Descansar N s / Siguiente serie». Migraciones M1/M2 **ya en LIVE** (`20260910205046`/`205101`, verificadas W6.11). W6 en curso (PostHog/Sentry, gates, docs). Orden de salida: OK del owner → push (`rnmobiledenuevo` + `master`) → deploy → avisos W6.13/W6.14 (DATA-TESTING §11) → OTA 1.1.2 android+ios → QA del owner 45 puntos (§7). El Playwright W6.10 queda verde recién con el seed corrido por el owner.
