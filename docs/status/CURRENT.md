...
|---|---|---|
| Web/PWA | Pricing v3 productivo: Free = 1 alumno + white-label + sello «Hecho con EVA»; Pro 25 sin sello. **Deploy vigente 11-09 02:52Z: `master` = `rnmobiledenuevo` = `091a19b0`, `dpl_6FXkTMyJ…` READY** — tren «Arreglos chicos pre-OTA» + F1 ([SDD](../specs/arreglos-chicos-pre-ota/SPEC.md); sin migraciones; E2E `prod-suave` 9/9 (run 34556450253, 42,7 s); **QA del owner pendiente**). Anterior 11-09 00:10Z `6321a732` / `dpl_BeykJxqv…` — «Cuenta atrás en pantalla» + D5 (QA VERDE salvo los 3 puntos con reps de F1). Deploys anteriores en el [historial](../archive/current-historial-2026-09.md). | [Runbook](../operations/RUNBOOK.md) · [spec](../specs/pricing-v3/SPEC.md) |
| App nativa (RN) | 1.1.2 es el piso OTA; canal `production` recibe android e ios por separado sobre el mismo commit. **OTA vigente 11-09 02:58Z** desde `master` @`091a19b0` (android `d4701f84` / ios `369ec7af`, runtime 1.1.2): «Arreglos chicos pre-OTA» + F1 ([SDD](../specs/arreglos-chicos-pre-ota/SPEC.md)); **QA del owner pendiente (3 puntos con reps de F1 + 11 del tren)**. Antes: 11-09 00:13Z @`6321a732` (Cuenta atrás + D5, QA VERDE salvo F1), 10-09 19:08Z @`27132cb9` (Señales honestas, `done`), 04:07Z @`d5d7d188` (marca cruzada). OTAs anteriores y QA de device en el [historial](../archive/current-historial-2026-09.md). | [Mobile parity](MOBILE_PARITY.md) · [OTA](../operations/MOBILE_RELEASES_OTA.md) |
