...
|---|---|---|
| Web/PWA | Pricing v3 productivo: Free = 1 alumno + white-label + sello «Hecho con EVA»; Pro 25 sin sello. **Deploy vigente 05-09 18:27Z: `master` = `rnmobiledenuevo` = `dff9b4fb`, `dpl_77CiJyBHVJ9VnzLWqwZ67u2PQF7A` READY** — fix de cookie corrupta (`auth-cookies` en server / admin-raw / proxy, que llenaba los logs), T9 `signOut` global, admin sin alumnos demo, `purge-data` pasa a diario, `rnmobiledenuevo` sin preview, `fflate` 0.8.3, migraciones reconstruidas y docs del QA verde. | [Runbook](../operations/RUNBOOK.md) · [spec](../specs/pricing-v3/SPEC.md) |
...
| Enterprise | **ELIMINADO de EVA (decisión del owner 2026-09-01)**: E0+E1 en código el 05-09 (app Expo, specs y scripts borrados); E2/E3 planificadas en el SDD. | [SDD retiro](../specs/retiro-starter-y-enterprise/SPEC.md) · [Ola de orden](../specs/ola-de-orden/TASKS.md) · [Flows](../architecture/FLOWS_AND_COMPONENTS.md#enterprise) |
...
1. **Tren «Ciclo real y por lado» (feedback Movens) — EN PRODUCCIÓN 04-09 02:25Z, QA del owner VERDE 04-09, SDD `done`** ([tareas](../specs/ciclo-real-y-por-lado/TASKS.md);
   ...
   texto en TESTING-QA §11; **enviado el 05-09**) y el E2E (W6.8), que se corre por GitHub Actions
   (`gh workflow run CI --ref master`, job `e2e`) en la sesión de gates: los secrets `E2E_*` viven en GitHub.
...
5. **(a) `EVA-NEXTJS-18` (hidratación en `/c/[coach_slug]/dashboard?recuperar=…`) — causa confirmada
   y FIX EN CÓDIGO 05-09 (O7.7), sin gates ni push**: el barrido O7.1 se salteó
   `WorkoutPlanCard.fmtShortDate` (client component del dashboard del ALUMNO); ahora usa
   `formatShortDayMonthEs` (tabla fija, cero `Intl`). **Verificar en Sentry a 72 h del deploy.**
...
6. **Errores al día (ola O6) — EN PRODUCCIÓN 01-09** (`master` `231d2937`, OTA 1.1.2 android
   ...
   EN CÓDIGO 05-09, sin gates ni push. [tareas § O6/O7](../specs/cierre-sentry-vivos/TASKS.md)
...
9. **Onboarding del coach v2 — W1–W4.7 y W5 parcial EN PRODUCCIÓN** ([spec](../specs/coach-onboarding-v2/SPEC.md),
   último `8cf7b886`): **W6 (correos por comportamiento) EN CÓDIGO 05-09, sin gates y sin push**, detrás de
   `ONBOARDING_BEHAVIOR_EMAILS_ENABLED` (default APAGADO; **no encender hasta aprobar el copy**) — F6.1 y F6.2
   hechos, F6.3 con tests escritos sin correr. D11 ejecutado: el drip por calendario queda apagado por defecto y
