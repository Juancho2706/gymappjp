...
|---|---|---|
| Web/PWA | Pricing v3 productivo: Free = 1 alumno + white-label + sello «Hecho con EVA»; Pro 25 sin sello. **Deploy vigente 11-09 22:54Z: `master` = `rnmobiledenuevo` = `e15a031c`, `dpl_5UDcjDBr…` READY** — F2 «Descanso siempre» ([SPEC §20](../specs/cuenta-atras-en-pantalla/SPEC.md)). Anteriores del 11-09 (`091a19b0` arreglos chicos + F1, `6321a732` cuenta atrás; QA del owner VERDE 11-09, SDD `done`) y previos en el historial. Deploys anteriores en el [historial](../archive/current-historial-2026-09.md). | [Runbook](../operations/RUNBOOK.md) · [spec](../specs/pricing-v3/SPEC.md) |
| App nativa (RN) | 1.1.2 es el piso OTA; canal `production` recibe android e ios por separado sobre el mismo commit. **OTA vigente 11-09 22:57Z** desde `master` @`e15a031c` (android `f0c0d18b` / ios `76bf4d26`, runtime 1.1.2): F2 «Descanso siempre» (QA VERDE 11-09). Antes 02:58Z @`091a19b0` (arreglos chicos + F1, QA VERDE 11-09), 00:13Z @`6321a732` (cuenta atrás), 10-09 @`27132cb9` y @`d5d7d188`. OTAs anteriores y QA de device en el [historial](../archive/current-historial-2026-09.md). | [Mobile parity](MOBILE_PARITY.md) · [OTA](../operations/MOBILE_RELEASES_OTA.md) |
...
0. **F2 «Descanso siempre» (reporte de un alumno, 11-09) — EN PRODUCCIÓN 11-09 22:57Z** (deploy `dpl_5UDcjDBr…` READY, OTA 1.1.2 android `f0c0d18b` / ios `76bf4d26`, E2E prod-suave 9/9; [SPEC §20](../specs/cuenta-atras-en-pantalla/SPEC.md) · [TASKS F2](../specs/cuenta-atras-en-pantalla/TASKS.md)): sin `rest_time` ya no se salta el descanso (fallback 60 s), el auto-avance espera al CTA con la pref OFF, roller/cardio RN con CTA, web offline descansa, `skip` huérfano Android descartado. **QA del owner VERDE 11-09 (artifact `53f6ec40`) ⇒ SDD cuenta-atrás `done`; con el mismo verde cierran arreglos-chicos-pre-ota, share-bloque y plan-vivo-y-guardado.** Siguiente: tren chico «Despegue rápido» (Sentry `EVA-NEXTJS-1P/1Q` + `EVA-MOBILE-F`, causa = waterfall de carga del ejecutor; B1 caché antes de auth en RN, B3 señal sin marca en web, TTL 20 s, telemetría honesta).
