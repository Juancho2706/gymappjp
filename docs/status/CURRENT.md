...
| Nutrition V2 | Canónica para Standalone/Team. En prod web: T2.1+T2.2 overrides, T2.3 hub de alimentos y **T2.4 sustituciones FULL** (merge `9dde4135`, 2026-08-10). **T2.5 swipe ⇄ + sheet 2 bloques: QA web + device CERRADO (2026-08-10)**, código en `rnmobiledenuevo` con 2 migraciones más en LIVE (`20260810161604`, `20260810171529`); incluye el fix de UI optimista del Hoy web (`5139b29e`) y cierra el reparo offline de T2.4 (chip "En cola"). **Pendiente decisión owner: merge a master + OTA android.** Quedan T2.6, T2.7 re-skin, T3.x editor único; preflight mantiene 7 enlaces V1→V2 por reconciliar | [Programa](../specs/nutrition-flows-redesign/TASKS.md) · [Runbook de corte](../operations/NUTRITION_V2_CUTOVER_RUNBOOK.md) |
...
2. Programa nutrición: **cerrar el despliegue de T2.5** (decisión owner: merge `rnmobiledenuevo` → master + OTA android) y triar las decisiones abiertas del QA: H1 (el buscador no encuentra nombres con paréntesis — asimetría de normalización con `name_search`, fix por el lado del catálogo) y D5 (pista de descubrimiento del swipe). Después T2.6. Paridad RN del tab Alimentos como tanda propia.
