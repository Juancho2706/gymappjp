...
0. **F2 «Descanso siempre» EN PRODUCCIÓN 11-09 22:57Z, QA del owner VERDE ⇒ `done`** (con el mismo verde cierran arreglos-chicos, share-bloque y plan-vivo; [SPEC §20](../specs/cuenta-atras-en-pantalla/SPEC.md)). **«Despegue rápido» EN CÓDIGO LOCAL, sin push** ([SDD](../specs/despegue-rapido/SPEC.md); Sentry `EVA-NEXTJS-1P/1Q` + `EVA-MOBILE-F`, causa = waterfall de carga del ejecutor): RN pinta desde caché antes de esperar auth, web emite la señal sin marca de morph, TTL 20 s, telemetría honesta. Sin migraciones. **Queda: OK del owner ⇒ push → deploy → OTA → QA ⇒ Sentry a 72 h.**
