...
0. **iOS 1.1.0 (52) RECHAZADA otra vez (2026-08-13)** — submission `912b9afb…`, revisada en iPad Air 11" (M3). Tres guidelines: **2.5.4** (`bluetooth-central` en `UIBackgroundModes` sin BLE en background), **2.5.1** (usa HealthKit sin identificarlo en la UI) y **1.4.1** (Nutrición entrega cálculos de salud sin citas). Los tres arreglados en código sobre `rnmobiledenuevo`; falta **build nueva + dos screen recordings en device físico** antes de reenviar | [Respuesta y checklist](../operations/app-review-1.1.0-respuesta-20260813.md)
