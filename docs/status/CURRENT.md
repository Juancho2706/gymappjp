...
0. **Tren «Reps tras el reloj» (fuerza por tiempo) — CÓDIGO LISTO Y JUZGADO, commits locales `da0525fe` (motor) · `170a6424` (RN) · `57fa541d` (web) sobre `9917f444` (SDD) y `b49cc0da` (fix del «teletransporte» del share), SIN PUSH** ([SDD](../specs/reps-tras-el-reloj/SPEC.md) · [tareas](../specs/reps-tras-el-reloj/TASKS.md); mockup aprobado artifact `6ead4180` v3): al llegar a 0 la serie se sigue guardando sola (V2), pero RN ahora suena como el descanso, y en las dos plataformas se abre solo el teclado/sheet si faltan reps (o kg), encima del descanso; línea «Serie N · 60 kg × 8 · 30 s · Editar · Repetir»; «Repetir» rearma el reloj y reemplaza la fila. Cero migraciones. Gates completos verdes 12-09 ([TEST_STATUS](../testing/TEST_STATUS.md)). **Queda: OK del owner ⇒ push → deploy → UNA OTA 1.1.2 android+ios (lleva también el fix del share) → E2E `prod-suave` → QA del owner (10 puntos del SPEC §10) ⇒ `done`.**
