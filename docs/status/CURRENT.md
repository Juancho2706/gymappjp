...
0. **Tren «Reps tras el reloj» (fuerza por tiempo) + Enmienda E1 — EN PRODUCCIÓN 12-09: `master` = `rnmobiledenuevo` = `9e153f23` (commits en [TEST_STATUS](../testing/TEST_STATUS.md)), deploy `dpl_5d2TczpX66aoGSsfAkkeqwW9BqVh` READY, OTA 1.1.2 android `01a097d4-0a6e-7bd9-8323-e7efd6cadeb0` / ios `01a097d4-2ebd-7a75-83c2-874454fd36fd`, E2E `prod-suave` 9/9 (run 34723934934)** ([SDD](../specs/reps-tras-el-reloj/SPEC.md) · [tareas](../specs/reps-tras-el-reloj/TASKS.md); mockup aprobado artifact `6ead4180` v3): a 0 la serie se guarda sola (V2), RN suena como el descanso, y si faltan reps (o kg) el teclado/sheet se abre solo SOBRE la pantalla del ejercicio con el descanso corriendo en la barra mini y un chip «Descanso 1:27»; al confirmar se expande el descanso con el mismo reloj (E1); línea «Serie N · Editar · Repetir»; «Repetir» rearma el reloj. Cero migraciones. Gates completos verdes 12-09 ([TEST_STATUS](../testing/TEST_STATUS.md)). **Queda: QA del owner en device y web (10 puntos del SPEC §10; incluye el arrastre del share) ⇒ SDD `done`.**
