...
> **Última actualización:** 2026-04-16 America/Santiago — Sprint 9/10: sistema de colores SYSTEM_PRIMARY_COLOR, NutritionModal responsive, 404 nav fix, emails legales, analytics dashboard, dots DayNavigator, goal weight, logo preview, dark mode suscripción, 404 page, aria-labels, RLS 24 tablas, build UTF-8 fix.

---

## Cambios recientes (delta)

### Sprint 9/10 — 2026-04-15/16 (Build fix + Analytics + Bug fixes)

- **Color system fix:** `src/lib/brand-assets.ts` → nueva constante `SYSTEM_PRIMARY_COLOR = '#007AFF'`. `coach/layout.tsx` → cuando `use_brand_colors_coach === false` usa `SYSTEM_PRIMARY_COLOR` en lugar de `BRAND_PRIMARY_COLOR` (#10B981 verde). Fallback `hexToRgb` corregido a `'0, 122, 255'`. `middleware.ts` → mismo fix para la vista del alumno. `BrandSettingsForm.tsx` → el cleanup del live-preview ya restauraba `#007AFF`; ahora alineado con el layout.
- **404 nav fix:** `CoachDashboardClient.tsx` → `href="/coach/nutrition"` → `href="/coach/nutrition-plans"`.
- **NutritionModal responsive:** `CoachDashboardClient.tsx` → `DialogContent` con `w-[95vw] sm:max-w-2xl md:max-w-4xl`; grid macros `grid-cols-2 md:grid-cols-4`; header `flex-wrap`; `MacroItem` simplificado sin `sm:flex-row`.
- **Emails legales:** `src/app/legal/page.tsx` y `src/app/privacidad/page.tsx` → `opcoach49@gmail.com` → `contacto@eva-app.cl`.
- **Dashboard analytics:** `DashboardCharts.tsx` (nuevo en `src/components/coach/dashboard/`) — AreaChart sesiones 30d + BarChart crecimiento alumnos. `dashboard.queries.ts` → `getMRRStats`, `getSessionsAreaData`, `getClientsBarData`, `getActivityFeed`. `CoachDashboardClient.tsx` → stat card MRR con delta, charts recargables, activity feed con tipos/color/thumbnail.
- **Dots actividad DayNavigator:** `actions.ts` (perfil alumno) → `getClientWorkoutActivityDates` + `getClientNutritionActivityDates`. `TrainingTabB4Panels.tsx` + `NutritionTabB5.tsx` → cargan dots on-mount, pasan a `DayNavigator`.
- **Goal weight:** migración `20260415120000_goal_weight_kg.sql` (`goal_weight_kg numeric` en `clients`). `updateClientGoalWeight` server action. Input inline en `ProgressBodyCompositionB6`. `WeightProgressChart` → `<ReferenceLine>` amarilla dashed.
- **KPI card polish:** `ClientProfileDashboard.tsx` → padding reducido, blur decorativo eliminado, "Racha Interact." duplicada removida.
- **Logo preview real-time:** `LogoUploadForm.tsx` → `URL.createObjectURL` + cleanup en unmount.
- **Dark mode suscripción:** `src/app/coach/subscription/page.tsx` → dark mode completo en cards, tabla e historial.
- **404 page:** `src/app/not-found.tsx` → nueva página 404 con branding EVA.
- **aria-labels:** añadidos en botones de ícono en sidebar coach, nav alumno, workout execution y exercise block.
- **RLS:** migración `20260414183000_superseded_preapproval_and_rls.sql` — RLS en 24 tablas. **Requiere `supabase db push`.**
- **Seguridad API:** email drip → Bearer exclusivo; `recipes/search` → rate limiting 30 req/min Upstash.
- **Performance:** `getFoodLibrary` + `getCoachFoodsCatalog` — SELECT * → columnas específicas.
- **Build UTF-8:** `database.types.ts` → reescrito de UTF-16 LE a UTF-8; añadidos `video_start_time`/`video_end_time` en tipo `exercises`.
...
└── DashboardContent (Server, _components/)
    ...
    └── CoachDashboardClient (Client)
        ...
        ├── Stat cards (MRR Estimado con delta %, Total Alumnos, Planes Activos)
        ├── NutritionModal + AdherenceModal (con datos en tiempo real)
        ├── Activity feed (check-in / workout / nuevo alumno, con thumbnail)
        └── DashboardCharts (AreaChart sesiones 30d + BarChart crecimiento alumnos)
```
