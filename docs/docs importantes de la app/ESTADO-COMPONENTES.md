...
**Última actualización:** 2026-04-16 America/Santiago (Sprint 9/10) — **Build fix UTF-8 types, color system SYSTEM_PRIMARY_COLOR, NutritionModal responsive, 404 nav fix, emails legales, dashboard analytics (MRR/AreaChart/BarChart/activity feed), dots DayNavigator, goal weight, KPI card polish, logo preview real-time, dark mode subscription, 404 page, aria-labels, RLS 24 tablas, recipes rate limiting, email drip Bearer.**
...
|------------|--------|---|-------|
| `page.tsx` (server) | 🔶 | 75% | Streaming con `Suspense` + `DashboardContent`; consulta principal movida a `_data/dashboard.queries.ts` (patrón estable). |
| `_data/dashboard.queries.ts` | ✅ | 95% | `Promise.all` paralelo + top 5 riesgo + actividades + charts + suscripción + **MRR mes actual/anterior + areaData sesiones 30d + barData crecimiento alumnos** (Sprint 9). |
| `CoachDashboardClient` | ✅ | 88% | Quick actions, alertas, charts. Banners trial/cancelado. **Sprint 9:** stat card MRR con delta %, `AreaChart` sesiones 30d, `BarChart` crecimiento alumnos 6 meses, activity feed con tipos/color/thumbnail. **Fix 2026-04-16:** 404 botón NUTRICION → `/coach/nutrition-plans`; NutritionModal fully responsive. |
| `NutritionModal` (fn en CoachDashboardClient) | ✅ | 90% | **Sprint 9/10:** `w-[95vw] sm:max-w-2xl md:max-w-4xl`, grid 2→4 cols, header `flex-wrap`, MacroItem sin corte de texto. |
| `DashboardCharts` | ✅ | 90% | Nuevo componente `src/components/coach/dashboard/DashboardCharts.tsx` — encapsula AreaChart + BarChart. |
...
| `actions.ts` | 🔶 | 65% | `getAdherenceStats`/`getNutritionStats` operativo. |
| `loading.tsx` | ✅ | 100% | Skeleton con stat cards + chart placeholder |

**Resultado del módulo: ~88% — Sprint 9: analytics completos (MRR, sesiones, crecimiento, activity feed), NutritionModal responsive. Pendiente: optimistic updates, offline banner.**
...
| `BrandSettingsForm` | ✅ | 78% | Formulario funcional con color, branding y `welcome_message`. Live preview del color vía CSS vars. **Fix 2026-04-16:** cleanup del efecto ahora restaura `#007AFF` (azul) correctamente (alineado con `coach/layout.tsx`). |
| `LogoUploadForm` | 🔶 | 65% | Upload a Supabase Storage + **preview en tiempo real con `URL.createObjectURL`** (Sprint 9). Limpieza en unmount. Sin crop. |
...
**Resultado del módulo: ~68% — Sprint 9: logo preview real-time. Fix color system azul/verde. Pendiente: preview moderno del dashboard alumno.**
