...
7. **Loaders = SI en el MVP, 6 variantes** provistas por el CEO en `specs/whitelabel-v2/loaders-source/` (DC-artifacts SVG+CSS, ya brand-aware via `--ld-brand`/`--ld-rgb`): **progreso, anillo, radar, cometa, ritmo, orbitas**. Son **opciones que el coach Pro+ elige**; el **DEFAULT (`loader_variant='eva'`) sigue siendo el loader actual de EVA** (el SVG multicolor existente) — lo ven free/starter y los Pro+ que no eligen otro. NO se reemplaza el actual. Fondo **theme-aware** (claro en light / oscuro en dark, NO el dark cosmico fijo de la demo); color fijado al `--theme-primary` del coach (el rainbow de la demo es solo muestra); icono central = logo del coach (icono EVA si no hay — **NEGRO en light, BLANCO en dark**); y los neutros del source (wordmark/pistas blancos) voltean a `--foreground` en light. W3 EN scope.
...
3. **Fondo + neutros theme-aware (decision CEO):** reemplazar el `#08080a` + glow + grid fijos por tokens que respeten el modo (`var(--background)` claro/oscuro; el glow usa `--theme-primary` a baja opacidad). NO forzar dark. **OJO los neutros tambien voltean:** el wordmark `#FFFFFF`, las pistas `rgba(255,255,255,0.07)` y el caption gris asumen fondo oscuro → en light usar `var(--foreground)`/tokens, sino quedan invisibles. Auditar TODO color claro hardcodeado del source.
4. **Icono central = logo del coach por modo** (`logo_url` en light / `logo_url_dark` en dark). Fallback EVA **tambien por modo**: **NEGRO en light** (`apps/web/public/LOGOS/` — `eva-icon.png` o `LOGO NEGRO SIN LETRAS SIN BG BORDE BLANCO.png`, confirmar cual es la marca negra al build) / **BLANCO en dark** (`eva-icon-white.png`). Un icono blanco sobre fondo claro queda invisible → por eso el par. La variante "ritmo" (05) no lleva icono.
...
8. **Wordmark:** el "EVA" del wordmark (variantes progreso/anillo) pasa a `brand_name` del coach, con color `var(--foreground)` (NO `#FFFFFF` fijo — ver task 3).
