...
- [x] **E3.1 [P] Descanso-interstitial** — fullscreen al cerrar serie (countdown anillo, ±15s/saltar, siguiente con media, micro-celebración, mensaje coach) + peek "Plan completo" arrastrable; timers actuales por debajo (endTime-based intactos).
- [x] **E3.2 [P] Movilidad por lado** — tono calmo (aqua en tema EVA); `side_mode='per_side'` → secuencia lado izq→haptic→der con HoldTimer; escribe `{left_sec,right_sec}` + agregado `actual_hold_sec`; `bilateral` = flujo actual.
- [x] **E3.3 [P] Roller** — contador pasadas gigante +1 (tick háptico, `reps_done` + `applyKeypadIncrement`), timer opcional.
- [x] **E3.4 [S] Cardio V3** — identidad (nombre+media catálogo, métrica por máquina), countdown `cardio-progress`, jerarquía N1/N2/N3, zonas fijas, FC manual, fases de intervalo (colores esfuerzo/recuperación, `buildIntervalPhases`, cues visuales; guard `isTimeableInterval` → stopwatch). **La zona objetivo del coach se muestra SIEMPRE y con rango bpm concreto** ("Z2 · 128-142 bpm", vía rangos de `@eva/cardio` + perfil FC del alumno): el alumno puede comparar contra su propio reloj con cualquier app; el BPM en vivo de Ola 6 es capa opcional encima, jamás requisito.
- [x] **E3.5 [P] Superserie V3** — rondas intercaladas (engine intacto), banner "sigue con B", momento ronda-cerrada + descanso de grupo, dots de ronda.
- [x] **E3.6 [P] Sustitución máquina ocupada RN** — portar `SubstituteExerciseSheet` (web ya la tiene); mismas actions/queries.
- [x] **E3.7 [S] Tuerca ajustes completa** — sheet: sonido cronómetro on/off + tono + volumen, vibración, celebraciones (OFF default), keep-awake, RPE/RIR visibles; persistencia device; en RN opciones de audio en gris "llega con actualización" hasta Ola 5; web suena ya (Web Audio actual).
- **Gate Ola 3**: ✅ CERRADA 2026-07-22 — suite completa verde, tsc ambos, tokens, docs. QA device/preview por tipo = pendiente CEO. Backlog honesto: sustitución en miembros de superserie no existe (tampoco en V2).
...
- [x] **E5.5 [P] Controles lockscreen Android (2026-07-31)** — resuelto SIN Media Session: acciones nativas en la notificación viva vía notify-kit (`Pausar / +15 s / Saltar` en descanso; `Pausar / Fase siguiente` en cardio) + handler headless `onBackgroundEvent` (registro ÚNICO compartido en `timers/notification-events.ts` — notify-kit admite UN solo background handler, despacho por prefijo `rest-*`/`cardio-*`) + snapshot/cola en `rest-remote-commands.ts`/`cardio-remote-commands.ts` (anti doble-apply por endEpochMs absoluto; 57 tests). Cardio además ganó su cronómetro vivo (`use-cardio-live-timer.ts`: countdown/fases/ascendente, color de zona, logo del coach) y su notif final (`cardio-notification.ts`, patrón QA-10). Deuda consciente: pausa desde lockscreen en intervalos/cronómetro cuenta el tiempo pausado como corrido (adoptar exige cirugía en `timing.ts`); iOS sigue en E7.2 (Live Activities).
...
- [~] **E7.1 [S] Módulo ActivityKit propio (CÓDIGO COMPLETO 2026-07-31 — Swift SIN COMPILAR hasta el build EAS #3)** — `@bacons/apple-targets@5.0.0` como plugin (`app.json`) + Widget Extension en `apps/mobile/targets/eva-timer-activity/` (`expo-target.config.js`, `deploymentTarget 16.2`, imageset de la figura EVA, App Group heredado de `ios.entitlements`) + módulo Expo LOCAL `apps/mobile/modules/eva-live-activity/` (autolink por `modules/`; `isSupported/start/update/end/drainCommands`, una activity por `kind` con `rest` y `cardio` coexistiendo). `app.json` gana `NSSupportsLiveActivities: true`, `NSSupportsLiveActivitiesFrequentUpdates: false` y el App Group `group.cl.evaapp.eva`. Puente JS con require guardado en `timers/live-activity.ts` (NO-OP total sin módulo nativo, en Android y en iOS < 16.2). **Decisión no obvia**: `EvaTimerAttributes` está DUPLICADO byte-idéntico (pod + `_shared` del target) porque son módulos Swift distintos y el pod no puede ser importado por la extensión ni importar la app; ambas copias llevan la marca "COPIA SINCRONIZADA". Falta: `ios.appleTeamId` y provisioning del extension en EAS.
- [~] **E7.2 [S tras E7.1] Live Activity + Dynamic Island (CÓDIGO COMPLETO 2026-07-31 — Swift SIN COMPILAR)** — `EvaTimerActivityWidget.swift`: tarjeta de lockscreen (fondo oscuro translúcido, título bold, subtítulo dim, `Text(timerInterval:)` monoespaciado tintado con el acento de marca) + Dynamic Island (compacta/mínima/expandida). Cubre descanso Y cardio (la Ola 5 dejó ambos cronómetros vivos en Android; iOS espeja los dos). Botones `LiveActivityIntent` en `_shared/EvaTimerIntents.swift` (compilan en el target de la APP y en el de la extensión, como exige Apple): descanso Pausar/+15 s/Saltar/Reanudar, cardio Pausar/Reanudar, con los MISMOS action ids que Android; cada intent mueve la actividad al instante y deja el comando en el App Group, que el JS drena en `AppState 'active'` (`timers/live-activity-commands.ts`, registrado en `app/_layout.tsx`) reconstruyendo el snapshot con el `atMs` del PRESS (17 tests nuevos). Botones sólo iOS 17+ (`#available`), degradación limpia sin ellos en 16.2–16.x. DIFERIDO consciente: logo del coach en iOS (exigiría cachear la imagen en el App Group) y "Fase siguiente" de cardio (el salto vive en el componente). Falta: build EAS #3 + QA en iPhone.
