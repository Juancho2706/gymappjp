...
- [ ] **T1 - Service: `unmarkCheckInReviewed`**
...
- [ ] **T2 - Test unitario de seguridad (DoD)**
...
- [ ] **T3 - Server action: `unmarkCheckInReviewed`**
...
- [ ] **T5 - Dashboard: `reviewed_at` en la query + `pendingCheckinsCount`**
...
- [ ] **T6 - Dashboard: badge + senal por item + filtro**
...
- [ ] **T7 - No-regresion metricas enterprise**
...
- [ ] `pnpm typecheck`
- [ ] `pnpm test` (vitest) de lo tocado; test de seguridad T2 verde
- [ ] Test de seguridad: un coach NO puede marcar NI des-marcar check-ins de alumnos ajenos (unit sobre la action/service, no E2E)
- [ ] No hay llamadas Supabase directas de feature-data en `_data` para las mutaciones (van por service)
- [ ] Server actions son solo async functions en el archivo `'use server'`
- [ ] Mutaciones llaman `revalidatePath()` donde corresponde
- [ ] Escritos usan cliente `authenticated` (nunca service-role) para heredar RLS
- [ ] Mobile viewport usa `dvh`, no `vh`/`h-screen`; UI fija usa safe-areas
- [ ] Dark mode revisado en badge, senal y toggle
- [ ] Sin migracion, sin GRANT, sin cambio de RLS (verificado: columnas/grant/policy ya existen)
- [ ] Docs canonicas actualizadas si cambia algo de rutas/flows (esta feature no agrega rutas)
