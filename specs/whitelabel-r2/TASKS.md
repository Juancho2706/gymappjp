...
**Fecha:** 2026-07-04
**Estado:** COMPLETADO 2026-07-04 (con re-alcances CEO)
...
- [x] **T0 — Apagar el auto-registro standalone en `/join`** `[M]`
...
- [x] **T3 — Refactor `InstallPrompt` + `PwaNavButton` al hook** `[M]`
...
- [x] **T4 — Emitir `data-primary-color`/`data-logo-url` + quitar `AppDownloadBanner` de `/c`** `[S]`
...
- [x] **T5 — Gating por primer workout + `appinstalled` + copy iOS 26** `[M]`
...
- [x] **T6 — Screenshots del manifest vía `next/og`** `[L]`
...
- [x] **T7 — Extender manifest per-coach y default** `[M]`
...
- [x] **T9 — `drawBrandFooter` compartido SIN QR + aplicarlo a las 4 cards** `[M]` *(re-alcance 2026-07-04 — nota: el footer luego se re-alcanzó de nuevo por CEO a "solo logo+nombre, sin URL")*
...
- [x] **T10 — `renderStreakCardToBlob` + `StreakShareCardModal`** `[M]`
...
- [x] **T11 — `_data/monthly-recap` + `renderMonthlySummaryCardToBlob` + modal** `[M]`
...
- [x] **T12 — Selector único en `ProfileClient`** `[M]`
...
- [x] `pnpm typecheck`
- [x] Tests dirigidos del dominio tocado (Vitest por tanda)
- [x] Sin llamadas Supabase directas de datos de feature en `_data` fuera del patrón vigente del módulo
- [x] Server actions validan con Zod (aplica al P0)
- [x] Mutaciones llaman `revalidatePath()` donde corresponde
- [x] Viewport móvil usa `dvh`, no `vh`/`h-screen`
- [x] UI fija de borde usa utilidades de safe-area
- [x] Dark mode revisado cuando cambia UI *(n/a el área imprimible del póster: Fase 5 diferida)*
- [x] Cero `<img>` raw (excepto dentro de `ImageResponse` de `next/og`)
- [x] Docs actualizados cuando cambian rutas, flujos, DB, tests o prioridades
