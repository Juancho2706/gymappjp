...
- [ ] **T1 — Extraer `SingleExerciseCard.tsx` con paridad 1:1** (M-L)
...
- [ ] **T3 — `WorkoutKeypadProvider.tsx`** (M)
...
- [ ] **T5 — Integrar keypad en `StrengthLogSetForm`** (M)
...
- [ ] **T6 — Ajuste de layout del keypad** (S)
...
- [ ] **T7 — QA cross-device del keypad** (M) — *gate CEO*
...
- [ ] **T8 — Helper puro `lib/workout-stepper.ts` + tests** (S-M)
...
- [ ] **T9 — `StepperExecution.tsx` (pager)** (L)
...
- [ ] **T10 — Persistencia del toggle stepper** (S)
...
- [ ] **T11 — UI del toggle + render condicional** (S-M)
...
- [ ] **T12 — Ramificar el auto-avance** (S-M)
...
- [ ] **T13 — A11y + reduced-motion + hardening del stepper** (S-M)
...
- [ ] **T14 — QA offline del stepper** (S) — *gate CEO*
...
- [ ] **T15 — Migración aditiva `workout_logs`** (S) — *aplicar en gate CEO*
...
- [ ] **T16 — `exercise-substitution.ts` (ranking puro) + tests** (M)
...
- [ ] **T17 — `_data/substitution.queries.ts`** (M)
...
- [ ] **T18 — `_actions/substitution.actions.ts`** (S-M)
...
- [ ] **T19 — Schema Zod de los 3 campos** (S)
...
- [ ] **T20 — `logSetAction` persiste los 3 campos** (S)
...
- [ ] **T21 — Offline queue: 3 campos opcionales** (S)
...
- [ ] **T22 — `SubstituteExerciseSheet.tsx`** (L)
...
- [ ] **T23 — Wiring en `WorkoutExecutionClient`** (M)
...
- [ ] **T24 — Thread de campos en `LogSetForm`** (S)
...
- [ ] **T25 — Rehidratación en `workout-execution.queries.ts`** (S)
...
- [ ] **T27 — QA/E2E del flujo de sustitución** (M) — *gate CEO*
...
- [ ] Vitest para lógica pura tocada (`workout-stepper`, `exercise-substitution`, `useCoarsePointer`, schema)
- [ ] Sin llamadas Supabase directas de feature-data en `_data/` (pasa por query→service/scope)
- [ ] Server actions validan con Zod (`getExerciseSubstitutionsAction`)
- [ ] Mutaciones llaman `revalidatePath()` donde aplique (persistencia reusa `logSetAction` existente)
- [ ] Mobile viewport usa `dvh`, no `vh`/`h-screen`
- [ ] UI fija en bordes usa safe-area (`pb-safe`/`pt-safe`)
- [ ] Dark mode verificado (la exec es dark siempre)
- [ ] Migración C: aditiva, idempotente (`IF NOT EXISTS`), forward-only, con snapshot `_bak` previo; SIN GRANT extra (GRANT de tabla) documentado en el comentario
- [ ] Docs actualizadas (`docs/architecture/FLOWS_AND_COMPONENTS.md`; `PROJECT_STRUCTURE.md` si aplica)
