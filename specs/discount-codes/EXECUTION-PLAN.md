...
#### Tasks (ordered)
1. **Header** mirroring `20260612150000_coach_addons_selfservice_billing.sql`: additive / idempotent / forward-only / NOT-APPLY-NOW; rollback block; **document dependency** that gate migrations `20260612140000` + `20260612150000` must land BEFORE this (critic: they're gate-pending; `billing_snapshots` discount-readiness and `coaches` grant allowlist come from them). Timestamp `20260620120000` is after latest `20260619140000` (critic: must replay in order).
...
8. `CREATE OR REPLACE FUNCTION sync_coach_active_coupon() ... SECURITY DEFINER` (mirror `sync_coach_enabled_modules`): recompute `coaches.active_coupon_redemption_id` from the live (`status='active'`) redemption, **COALESCE to NULL** when none (critic: `enabled_modules '{}'` gotcha). AFTER INSERT/UPDATE/DELETE trigger on `coupon_redemptions`.
...
10. **`resolve_active_discount(coach_id uuid)` SECURITY DEFINER RPC** (critic blocker — `subscription-status` reads with USER-SCOPED client; a join to `coupons`/`coupon_codes` returns empty under RLS → page shows list price while webhook charges discounted): returns ONLY the frozen `discount_value_snapshot` + `applied_cycles_remaining` from the coach's live redemption. `REVOKE EXECUTE FROM anon`; `GRANT EXECUTE TO authenticated` scoped to own coach.
11. Btree indexes: `coupon_redemptions(coach_id)`, `(coupon_id)`, `(status)`; `coupon_codes(coupon_id)`; FK-covering index on `coaches.active_coupon_redemption_id`. Verify `EXPLAIN` of code lookup hits the unique partial idx.
