/**
 * App Review (Apple) — cuenta de ALUMNO demo CARGADA de datos sobre el coach `evademo`.
 *
 * POR QUE EXISTE: Apple rechazo la app iOS por guideline 2.1(a) (Performance — App Completeness):
 * el revisor no pudo entrar como alumno y ver la app funcionando. Este seed deja una cuenta de
 * alumno lista para pegar en las "App Review Information" (Sign-In required) con contenido real
 * en TODAS las superficies del alumno: dashboard (peso, racha, habitos), rutina + historial de
 * entrenamiento, nutricion, check-ins, movimiento (FMS) y composicion corporal (BIA + ISAK).
 *
 * QUE CREA / ASEGURA (todo idempotente — crea SOLO lo que falta):
 *   0. Modulos del coach demo: filas vivas en `coach_addons` (source='admin_grant', price_clp=0)
 *      para los 4 modulos. El trigger `trg_coach_addons_sync` recomputa `coaches.enabled_modules`
 *      en la misma transaccion — NUNCA se escribe ese jsonb a mano (esta bloqueado por column-grant).
 *      Sin esto, medio app le aparece apagada al revisor.
 *   1. Cuenta auth + `clients` + `client_accounts` + `client_memberships` (scope standalone) +
 *      consentimiento de datos de salud.
 *   2. Contenido: intake, programa de 3 dias con bloques y ~12 sesiones logueadas, plan de
 *      nutricion con 4 comidas + 10 dias de adherencia, 9 check-ins, 14 dias de habitos,
 *      3 mediciones BIA + 2 ISAK, y 2 evaluaciones FMS finales con sus 7 items.
 *
 * APUNTA A PROD DELIBERADAMENTE (mismo target que seed-josefit-demo-alumno). Doble gate:
 *   1) flag --allow-remote
 *   2) env APPREVIEW_SEED_CONFIRM=yes
 *
 * Uso (PowerShell):
 *   $env:APPREVIEW_SEED_CONFIRM='yes'; $env:APPREVIEW_ALUMNO_PASSWORD='<password>'; node scripts/seed-appreview-demo.mjs --allow-remote
 * Uso (bash):
 *   APPREVIEW_SEED_CONFIRM=yes APPREVIEW_ALUMNO_PASSWORD='<password>' node scripts/seed-appreview-demo.mjs --allow-remote
 *
 * Password del alumno: env APPREVIEW_ALUMNO_PASSWORD (o E2E_PERSONAS_PASSWORD como fallback).
 *
 * IDEMPOTENTE: re-correrlo no duplica nada (solo re-setea el password del alumno). Cuenta
 * @evatest.cl => permanente, excluida de purgas de cuentas de prueba.
 * Shapes verificados contra apps/web/src/lib/database.types.ts, la migracion
 * 20260612150000_coach_addons_selfservice_billing.sql y los repos de bodycomp/movement.
 *
 * OJO (nutricion): este seed escribe nutricion V1 (`nutrition_plans` + `nutrition_meals` +
 * `food_items` + `daily_nutrition_logs`), igual que el seed probado de josefit. La superficie del
 * ALUMNO hoy es Nutricion V2 (`/c/<slug>/nutrition` redirige a `/nutrition-v2`; mobile lee
 * `get_nutrition_today_v2` sobre las tablas `*_v2`). Si el revisor debe ver el plan alimenticio
 * lleno, hay que publicarle ademas un plan V2 desde el panel del coach demo. El script avisa al
 * final si el alumno NO tiene plan V2 publicado.
 */
...
const COACH_ID = '7f1a9119-cca6-420d-ae47-45d8e401bfda' // evademo (EVA Demo), standalone, tier pro
const ALUMNO_EMAIL = 'appreview-alumno@evatest.cl'
const ALUMNO_NAME = 'Alex Demo'
const ALUMNO_PASSWORD = process.env.APPREVIEW_ALUMNO_PASSWORD || process.env.E2E_PERSONAS_PASSWORD
if (!ALUMNO_PASSWORD || ALUMNO_PASSWORD.length < 8) {
    throw new Error('Falta APPREVIEW_ALUMNO_PASSWORD o E2E_PERSONAS_PASSWORD (mínimo 8 caracteres).')
}

const PROGRAM_NAME = 'Hipertrofia 3 dias'
const NUTRITION_PLAN_NAME = 'Plan Recomposicion'

/**
 * Los 4 modulos de pago (espejo de MODULE_KEYS en apps/web/src/services/entitlements.service.ts
 * y del CHECK de `coach_addons.module_key`). `terms_version='admin_grant'` es el centinela de las
 * cortesias del CEO (ADMIN_GRANT_TERMS_VERSION en services/billing/addons.service.ts).
 */
const MODULE_KEYS = ['cardio', 'movement_assessment', 'body_composition', 'nutrition_exchanges']
const ADMIN_GRANT_TERMS_VERSION = 'admin_grant'
...
async function ensureUser(admin, email, fullName, password) {
    ...
    const { data, error } = await admin.auth.admin.createUser({
        ...
        password,
        email_confirm: true,
        user_metadata: { full_name: fullName, demo_seed: 'appreview' },
    })
...
/**
 * Modulos del coach demo (paso EXTRA respecto del seed de josefit, que asume el coach ya
 * configurado). Mecanismo REAL: filas en `coach_addons`; el trigger D1 recomputa
 * `coaches.enabled_modules`. Solo se insertan los modulos SIN fila viva (active|cancel_pending)
 * — si ya hay una fila `self_service` viva el modulo ya esta ON y no se agrega cortesia encima.
 * Este script NUNCA cancela ni toca filas existentes.
 */
async function ensureCoachModules(admin) {
    const live = must(
        await admin
            .from('coach_addons')
            .select('id, module_key, source, status')
            .eq('coach_id', COACH_ID)
            .in('status', ['active', 'cancel_pending']),
        'coach_addons select'
    )
    const liveKeys = new Set(live.map((r) => r.module_key))
    const missing = MODULE_KEYS.filter((k) => !liveKeys.has(k))
    const already = MODULE_KEYS.length - missing.length
    if (already > 0) track('coach_addons', false, already)
    if (missing.length === 0) {
        console.log('  = modulos: los 4 ya tienen fila viva en coach_addons — skip')
        return
    }
    must(
        await admin.from('coach_addons').insert(
            missing.map((key) => ({
                coach_id: COACH_ID,
                module_key: key,
                status: 'active',
                source: 'admin_grant',
                price_clp: 0, // cortesía: NUNCA factura (D2)
                terms_version: ADMIN_GRANT_TERMS_VERSION,
            }))
        ),
        'coach_addons insert'
    )
    track('coach_addons', true, missing.length)
    console.log(`  + modulos concedidos (admin_grant, price 0): ${missing.join(', ')}`)
}
...
async function seedIntake(admin, clientId) {
    ...
    must(
        await admin.from('client_intake').insert({
            ...
            weight_kg: 78,
            height_cm: 178,
            goals: 'Recomposicion corporal: bajar grasa y ganar musculo.',
    ...
    const plan = must(
        await admin
            ...
            .insert({
                ...
                carbs_g: 210,
                fats_g: 70,
                instructions: 'Cuatro comidas al dia. Prioriza proteina en cada comida.',
            })
            .select('id')
            .single(),
...
async function seedCheckIns(admin, clientId) {
    ...
    // 9 check-ins semanales — peso descendente (recomposicion), energia variada.
    const series = [
    ...
    const rows = series.map(({ daysAgo, weight, energy }, i) => ({
        ...
        weight,
        energy_level: energy,
        notes: `Check-in ${i + 1} — sintiendome bien, progreso constante.`,
        ...
        const assessment = must(
            await admin
                ...
                .insert({
                    ...
                    notes: 'Screening de movimiento (FMS).',
...
/**
 * Chequeo SOLO-LECTURA: ¿el alumno tiene plan de Nutricion V2 publicado? La superficie del alumno
 * (web `/nutrition-v2`, mobile `get_nutrition_today_v2`) lee las tablas `*_v2`, que este seed NO
 * escribe. Devuelve true/false; nunca revienta el seed (si la lectura falla, avisa y sigue).
 */
async function hasPublishedNutritionV2Plan(admin, clientId) {
    const { data, error } = await admin
        ...
        .limit(1)
    if (error) {
        console.warn(`  ! no se pudo verificar nutricion V2: ${error.message}`)
        return null
    }
...
async function main() {
    ...
    if (!process.argv.includes('--allow-remote') || process.env.APPREVIEW_SEED_CONFIRM !== 'yes') {
        console.error(
    ...
    // Verificar coach demo
    const coach = must(
        await admin
            .from('coaches')
            .select('id, slug, brand_name, subscription_tier, subscription_status, enabled_modules')
            .eq('id', COACH_ID)
            .maybeSingle(),
        'coaches verify'
    )
    if (!coach) throw new Error(`Coach demo ${COACH_ID} no existe en prod`)
    console.log(
        `Coach OK: slug=${coach.slug} brand="${coach.brand_name}" tier=${coach.subscription_tier} ` +
        `status=${coach.subscription_status} modulos=${JSON.stringify(coach.enabled_modules)}`
    )

    // Modulos del coach demo (coach_addons -> trigger -> coaches.enabled_modules)
    console.log('== Modulos del coach demo ==')
    await ensureCoachModules(admin)
    const coachAfter = must(
        await admin.from('coaches').select('enabled_modules').eq('id', COACH_ID).maybeSingle(),
        'coaches re-read'
    )
    console.log(`  enabled_modules ahora: ${JSON.stringify(coachAfter?.enabled_modules ?? {})}`)
    ...
    const v2Plan = await hasPublishedNutritionV2Plan(admin, user.id)

    console.log('\n=== LISTO ===')
    console.log(JSON.stringify({
        login_url_web: `https://www.eva-app.cl/c/${coach.slug}/login`,
        login_app: 'App EVA (iOS/Android) -> Iniciar sesion con este email y password',
        email: ALUMNO_EMAIL,
    ...
    if (v2Plan === false) {
        console.warn(
            '\n⚠️  El alumno NO tiene plan de Nutricion V2 publicado. La app del alumno lee V2 ' +
            '(get_nutrition_today_v2 / /nutrition-v2): el plan V1 que sembro este script NO se le ' +
            'muestra. Publica un plan desde el panel del coach demo antes de mandarlo a App Review.'
        )
    }
}
