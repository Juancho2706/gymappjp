...
export async function validateImageDimensions(buffer: Buffer): Promise<DimensionsResult> {
    // Dynamic import keeps sharp out of any bundle analysis pass that runs before tree-shaking.
    // The `import 'server-only'` guard above ensures this file is never included in client bundles.
