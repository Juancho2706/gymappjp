...
import { toggleMealCompletion } from '@/app/c/[coach_slug]/nutrition/actions'
...
export function MealCompletionRow({
    ...
    const onToggle = () => {
        ...
        startTransition(async () => {
            setOptimistic(next)
            await toggleMealCompletion(clientId, planId, mealId, next, dailyLogId, coachSlug)
            router.refresh()
        })
    }
    ...
    return (
        <button
            ...
            disabled={pending}
            onClick={onToggle}
            className="flex w-full items-center gap-3 rounded-xl border border-border/50 bg-card/40 px-3 py-2 text-left transition-colors hover:bg-accent/30"
        >
            <span
